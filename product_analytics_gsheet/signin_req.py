...
def sign_in():
    ...
    'password': 'cEf&ce$6fist8Tr=3R+9',
}

    response = requests.post('https://api.factors.ai/agents/signin', headers=headers, json=json_data)

    sub1="factors-sid="
    sub2=";"

    idx1 = response.headers['Set-Cookie'].index(sub1)
    idx2 = response.headers['Set-Cookie'].index(sub2)

    factors_sid=""

    for idx in range(idx1 + len(sub1), idx2):
        factors_sid = factors_sid + response.headers['Set-Cookie'][idx]
