...
import { Row, Col, Button, Spin, Tag, Input } from 'antd';
import {
  fetchFactorsGoals,
  fetchFactorsModels,
  fetchGoalInsights,
  ...
  return (
    <div className={`flex items-center justify-between explain-table--row`}>
      <div className={`py-2 px-4 flex items-center`}>
        <SVG
          name={isIncreased ? 'spikeup' : 'spikedown'}
        ...
        <Text
          ...
          level={6}
          weight={'bold'}
          extraClass={'m-0 ml-2 capitalize'}
        >
          {title}
        </Text>
      </div>
...
const InsightItem = ({
  ...
  return dataSet?.map((item) => {
    if (
      (item?.factors_insights_type == 'journey' ||
        item?.factors_insights_type == 'campaign') &&
      !isAttribute
    ) {
      if (
        item?.factors_multiplier_increase_flag == showIncrease &&
        item?.factors_insights_key.toLowerCase().includes(searchTerm)
      ) {
        return (
          <div
            className={`flex items-center justify-between cursor-pointer explain-table--row`}
            onClick={() => {
              setModalL2(true);
              setModalData(item);
            }}
          >
            ...
            <div className={'py-2 px-4 flex justify-end column_right'}>
              <Tag
                color={item?.factors_multiplier_increase_flag ? 'green' : 'red'}
                className={`m-0 mx-1 ${
                  item?.factors_multiplier_increase_flag
                    ? 'fa-tag--green'
                    : 'fa-tag--red'
                }`}
              >
                <Number
                  suffix={'%'}
                  number={item?.factors_insights_percentage}
                />
              </Tag>
            </div>
          </div>
      ...
      <Col span={24}>
        <div className={'my-5'}>
          <div className={'border--thin-2  border-radius--sm'}>
            <div
              className={
                'py-4 px-6 background-color--brand-color-1 border-radius--sm flex justify-between'
              }
            >
              <Text
                type={'title'}
                ...
                extraClass={'m-0 capitalize'}
              >{`${
                isAttribute ? 'Segments (Attributes)' : `Engagements (Journeys)`
              }`}</Text>
          ...
          <>
            {true ? (
              <>
                <div
                  id={`explain-results`}
                  className={'my-6 w-full flex justify-center mt-10'}
                >
