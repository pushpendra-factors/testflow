...
const MapComponent = ({
  ...
  const handleChange = useCallback(
    (value: String, index: any, type: String) => {
      let obj = {};
            ...
            <div className='flex items-center mt-2'>
              ...
              {!limit && (
                <div className='ml-2'>
                  <Button
                    size={'small'}
                    icon={<SVG name='trash' size={14} />}
                    onClick={() => deletePropertyMap(index)}
                  />
                </div>
              )}
            </div>
  ...
  return (
    <>
      <div className='flex flex-col justify-start'>{renderDropdown()}</div>
      {!limit && (
        <Button
          className='mt-4'
          style={{ width: '200px' }}
          onClick={() => setPropertyMap([...propertyMap, {}])}
        >
          Add new
        </Button>
      )}
