...
import {linkedInScope_rw_ads, linkedInScope_rw_conversions} from './constants';
...
const LinkedInIntegration = ({
  ...
  const renderLinkedinLogin = () => {
    if (!currentProjectSettings?.int_linkedin_access_token) {
      ...
      let scope_rw_ads = linkedInScope_rw_ads.includes(activeProject?.id);
      let scope_rw_conversions = linkedInScope_rw_conversions.includes(activeProject?.id);
      //linkedIn oauth url gets updated based on the scopes assigned to project.
      let href = `https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=${BUILD_CONFIG.linkedin_client_id}&redirect_uri=${redirect_uri}&state=factors&scope=r_basicprofile%20r_liteprofile%20r_ads_reporting${scope_rw_ads?'%20rw_ads':'%20r_ads'}${scope_rw_conversions?'%20rw_conversions':''}`;
      return (
        <a href={href} className='ant-btn ant-btn-primary'>
          {' '}
          Enable using LinkedIn{' '}
        </a>
      );
    }
  };
                ...
                <Row>
                  <Col span={24}>
                    ...
                    <Text
                      ...
                      extraClass={'m-0 mt-2'}
                    >
                      Choose your LinkedIn Ad account to sync reports with
                      Factors for performance reporting
                    </Text>
                  </Col>
                </Row>
