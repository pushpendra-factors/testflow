...
const FacebookIntegration = ({
      ...
      fetch(`https://graph.facebook.com/v13.0/${response.id}/adaccounts?access_token=${response.accessToken}`)
        .then(res => res.json().then((r) => {
          if (r.data?.length != 0) {
            ...
            message.error("You don't have any ad accounts associated to the id you logged in with.");
          }
        }))
  ...
  const handleSubmit = e => {
    ...
    if (SelectAdAccount != "") {
      ...
      addFacebookAccessToken(data).then(() => {
        ...
        return (
          ...
          >
            <div className={'p-4'}>
              <Row>
                <Col span={24}>
                  <Text type={'title'} level={6} weight={'bold'} extraClass={'m-0'}>Choose your Facebook Ad account:</Text>
                  <Text type={'title'} level={7} color={'grey'} extraClass={'m-0 mt-2'}>Choose your Facebook Ad account to pull in reports from Facebook, Instagram and Facebook Audience Network</Text>
                </Col>
              </Row>
