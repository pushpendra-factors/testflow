...
import { Row, Col, Modal, Input, Select, Form, Button, message } from 'antd';
...
function InviteUsers(props) {
  ...
  const inviteUser = (payload) => {
    ...
    seterrorInfo(null);
    props
      .projectAgentInvite(props.activeProjectID, payload)
      .then(() => {
            ...
            <Row gutter={[24, 24]}>
              ...
              <Col span={8}>
                ...
                <Form.Item
                  name='role'
                  rules={[
                    { required: true, message: 'Please choose user role' }
                  ]}
                  className={'m-0'}
                >
