...
function ContentGroupsForm({
  ...
  const renderRuleViewButtons = (rules) => {
    return rules.map((obj, i) => {
      return (
        <div className={`flex justify-start -mr-48 ${i > 0 && 'mt-4'}`}>
          <Button
            type={'text'}
            size={'large'}
            style={{ color: 'gray' }}
  ...
  const columns = [
    {
      ...
      render: (text) => (
        <span className={'text-gray-600 text-sm font-bold'}>{text}</span>
      )
    },
    {
      title: 'Rule',
  ...
  const confirmRemove = (obj) => {
    if (formState == 'add') {
      const rulesToUpdate = [
        ...rulesState.filter((rl) => JSON.stringify(rl) !== JSON.stringify(obj))
      ];
      setRulesState(rulesToUpdate);
    }

    const rulesToUpdate = [
      ...smartPropState.rule.filter(
  ...
  const createForm = (smrtProp) => {
    addContentGroup(activeProject.id, smrtProp).then(
      (res) => {
  ...
  const updateForm = (smrtProp) => {
    updateContentGroup(activeProject.id, smrtProp).then(
      (res) => {
        smrtProp.id = res.data.id;
        setSmartPropState({ ...smrtProp });
  ...
  const onFinish = (data) => {
    if (data) {
      ...
      setShowSmartProperty(false);
      const smrtProp = {
        id: smartPropState.id ? smartPropState.id : '',
        content_group_name: data.content_group_name,
        content_group_description: data.content_group_description,
        rule: rulesState
      };
      if (formState !== 'add') {
        updateForm(smrtProp);
  ...
  const handleValuesSubmit = (data, oldRule) => {
    if (data) {
      ...
      if (formState === 'view') {
        const smrtProp = {
          id: smartPropState.id,
          project_id: smartPropState.project_id,
          content_group_name: smartPropState.content_group_name,
          content_group_description: smartPropState.content_group_description,
          rule: rulesToUpdate
        };
        updateForm(smrtProp);
      }
  ...
  useEffect(() => {
    ...
    rulesState.forEach((rl) => {
      columData.push({
        content_group_value: rl.content_group_value,
        rule: rl.rule,
        actions: rl
      });
    });
      ...
      <>
        ...
        <Row className={'mt-6'}>
          <Col span={18}>
            ...
            <Text type={'title'} level={6} extraClass={'m-0'} weight={'bold'}>
              {smartPropState.content_group_description}
            </Text>
            {formState === 'view' ? (
              <Button
                size={'large'}
        ...
        <Col span={24}>
          <div>
            <Form
              ...
              onChange={onChange}
              initialValues={{
                content_group_name: selectedGroup?.content_group_name
                  ? selectedGroup.content_group_name
                  : '',
                content_group_description:
                  selectedGroup?.content_group_description
                    ? selectedGroup.content_group_description
                    : ''
              }}
            >
              ...
              {formState !== 'view'
                ? renderContentGroupForm()
                : renderContentGroupDeatails()}
              ...
              <Row className={'mt-6'}>
                ...
                <Col span={12}>
                  <div
                    className={'flex justify-end border-top--thin pt-5 mt-5'}
                  >
                    <Button
                      type={'text'}
                      size={'middle'}
                    ...
                    >
                      <SVG name={'plus'} extraClass={'m-0'} size={18} />
                      New value
                    </Button>
                  </div>
                </Col>
                ...
                <Row className={'mt-8'}>
                  <Col span={24}>
                    ...
                    <Text
                      ...
                      level={7}
                      color={'grey'}
                      extraClass={'m-0 flex justify-center mt-4'}
                    >
                      Create Values that defines this content group
                    </Text>
