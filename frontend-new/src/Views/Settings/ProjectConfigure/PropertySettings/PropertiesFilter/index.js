  ...
  const renderPropSelect = () => {
    return (
      <div className={`${styles.filter__propContainer}`}>
        <Button
          icon={
            propState && propState.icon ? (
              <SVG name={propState.type} size={16} color={'purple'} />
            ) : null
          }
        ...
        >
          {' '}
          {propState?.name ? propState?.name : 'Select Property'}
        </Button>
    ...
    return (
      <div className={`${styles.filter__propContainer} ml-2`}>
        <Button
          ...
          type='link'
          onClick={() => setOperSelectOpen(!operSelectOpen)}
        >
          {' '}
          {operatorState
            ? reverseOperatorMap[operatorState]
            : 'Select Operator'}
        </Button>
