...
function PropertyValueModal({
  ...
  useEffect(() => {
    if (config) {
      const src = config.sources
        ? config.sources.filter((sr) => sr.name === selectedSource)
        : [];
    ...
    filters.forEach((fil, i) => {
      filterElements.push(
        <div>
          <PropertyFilter
            filter={fil}
            propOpts={propertyOpts}
            insertFilter={(filt) => insertFilter(i, filt)}
          ></PropertyFilter>
  ...
  const renderSelectFilter = () => {
    const filterSelect = !showNewFilter ? (
      ...
      <PropertyFilter
        propOpts={propertyOpts}
        insertFilter={(filt) => insertFilter(-1, filt)}
      ></PropertyFilter>
    );
    return filterSelect;
  };
  ...
  const renderSourceOptions = () => {
    ...
    return sources.map((source) => {
      return (
        <Option value={source} className={`capitalize`}>
          {' '}
          {source}{' '}
        </Option>
      );
    ...
    >
      <Form
        ...
        initialValues={{
          ...
          combOperator:
            rule?.filters && rule.filters[0].logical_operator
              ? rule.filters[0].logical_operator
              : 'AND'
        }}
      >
        <Row className={'mt-8'}>
        ...
        <Row className={'mt-8'}>
          <Col span={24}>
            <div className='flex justify-end'>
              <Button size={'large'} disabled={false} onClick={handleCancel}>
                Cancel
              </Button>
              <Button
                size={'large'}
