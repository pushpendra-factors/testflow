...
function SmartProperties({ activeProject,
    setShowSmartProperty, smartProperty,
    fetchSmartPropertiesConfig, config,
    addSmartProperty, updateSmartProperty
}) {
    ...
    const renderFilterViewButtons = (filters) => {
        return filters.map((obj, i) => {
            return (<div className={`flex justify-start ${i > 0 && 'mt-4'}`}>
                <Button
                    icon={obj && obj.name ? <SVG name={obj.name} size={16} color={'grey'} /> : null}
                    className={`fa-button--truncate`}
                    style={{ pointerEvents: "none" }}> {obj?.property}
                </Button>

                <Button
                    className={`fa-button--truncate ml-4`}
                    style={{ pointerEvents: "none" }}> {reverseOperatorMap[obj?.condition]}
                </Button>

                <Button
                    className={`fa-button--truncate ml-4`}
                    style={{ pointerEvents: "none" }}> {obj?.value}
                </Button>

                {(i + 1 === filters.length) ? null : <Text type={'title'} level={7} extraClass={'ml-2'}>{obj.logical_operator}</Text>}
            </div>)
        })
    }
    ...
    const confirmRemove = (obj) => {
        const rulesToUpdate = [...smartPropState.rules.filter((rule) => JSON.stringify(rule) !== JSON.stringify(obj))];
    ...
    useEffect(() => {
        ...
        rulesState.forEach((rl) => {
            columData.push({ value: rl.value, source: rl.source, rule: rl.filters, actions: rl });
        })
    ...
    const createForm = (smrtProp) => {
        addSmartProperty(activeProject.id, smrtProp).then(res => {
    ...
    const updateForm = (smrtProp) => {
        updateSmartProperty(activeProject.id, smrtProp).then(res => {
            smrtProp.id = res.data.id;
            setSmartPropState({ ...smrtProp });
    ...
    const handleValuesSubmit = (data, oldRule) => {
        if (data) {
            const valueFilters = [...data.filters.map((fl) => {
    ...
    const renderPropertyForm = () => {
        return (<>
            <Row className={'mt-8'}>
                <Col span={18}>
                    <Text type={'title'} level={7} extraClass={'m-0'}>Type</Text>
                    <Form.Item
                        name="type"
                    >
                        <Select className={'fa-select w-full'} size={'large'} onChange={(val) => {
                            if (val !== config.name) {
            ...
            <Row className={'mt-8'}>
                <Col span={18}>
                    ...
                    >
                        <Input value={smartPropState.name} size="large" className={'fa-input w-full'} placeholder="Display Name" />
                    </Form.Item>
                </Col>
            </Row>
            ...
            <Row className={'mt-8'}>
                <Col span={18}>
                    ...
                    >
                        <Input value={smartPropState.description} size="large" className={'fa-input w-full'} placeholder="Description" />
                    </Form.Item>
                </Col>
            </Row>
    ...
    const renderPropertyDetail = () => {
        return (<>
            <Row className={'mt-8'}>
                <Col span={12}>
                    <Text type={'title'} level={7} extraClass={'m-0'}>Type</Text>
                    <Text type={'title'} level={6} extraClass={'m-0 capitalize'} weight={'bold'}>{smartPropState.type_alias.replace('_', ' ')}</Text>
                </Col>
                <Col span={12}>
                    <Text type={'title'} level={7} extraClass={'m-0'}>Display Name</Text>
                    <Text type={'title'} level={6} extraClass={'m-0'} weight={'bold'}>{smartPropState.name}</Text>
                </Col>
            </Row>
            ...
            <Row className={'mt-8'}>
                <Col span={18}>
                    <Text type={'title'} level={7} extraClass={'m-0'}>Description </Text>
                    <Text type={'title'} level={6} extraClass={'m-0'} weight={'bold'}>{smartPropState.description}</Text>
                </Col>
            </Row>
            ...
            <Col span={24}>
                <div>
                    <Form
                        ...
                        initialValues={
                            {
                                type: smartProperty?.type_alias ? smartProperty.type_alias : 'campaign',
                                description: smartProperty?.description ? smartProperty.description : '',
                    ...
                    >
                        ...
                        <Row>
                            ...
                            <Col span={12}>
                                <div className={'flex justify-end'}>
                                    <Button size={'large'} disabled={false} onClick={() => setShowSmartProperty(false)}>Cancel</Button>
                                    {
                                        formState === 'view' ?
                                            <Button size={'large'} disabled={false} className={'ml-2'} onClick={() => setFormState('edit')}>Edit</Button>
                                            : null
                                    }
                                    {
                        ...
                        {formState !== 'view' ? renderPropertyForm() : renderPropertyDetail()}
    ...
    const renderValuesTable = () => {
        return (
            <>
                <Row className={`mt-8`}>
                    <Col span={12}>
                        <Text type={'title'} level={5} weight={'bold'} extraClass={'m-0'}>Values</Text>
                    </Col>
                    <Col span={12}>
                        <div className={'flex justify-end'}>
                            <Button size={'medium'} onClick={() => setShowModalVisible(true)}><SVG name={'plus'} extraClass={'mr-2'} size={16} />New Value</Button>
                        </div>
                    </Col>
                </Row>
                <Row>
    ...
    const renderValuesModal = () => {
        ...
        return <PropetyValueModal config={config} rule={selectedRule} sources={valueSources} handleCancel={handleCancel} submitValues={handleValuesSubmit}></PropetyValueModal>
    }
...
export default connect(mapStateToProps, { fetchSmartPropertiesConfig, addSmartProperty, updateSmartProperty })(SmartProperties);