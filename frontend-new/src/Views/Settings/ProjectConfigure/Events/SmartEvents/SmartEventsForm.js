...
import {
  saveSmartEvents,
  fetchSmartEvents,
  fetchObjectPropertiesbySource,
  fetchSpecificPropertiesValue
...
function SmartEventsForm({
  ...
  const [timestampReferenceOthers, setTimestampReferenceOthers] =
    useState(false);
  ...
  const [objPropertiesSourceArrDT, setobjPropertiesSourceArrDT] =
    useState(null);
  ...
  const inputComponentRef = useAutoFocus(!loading && formState !== 'view');
  ...
  const onFinish = (data) => {
    ...
    const finalData = {
      ...
      expr: {
        ...
        timestamp_reference_field:
          data.timestamp_reference_field === 'other'
            ? data.datetime_objProperty
            : data.timestamp_reference_field,
        filters: [
          {
            logical_op: 'AND',
    ...
    saveSmartEvents(activeProject.id, finalData)
      .then((data) => {
  ...
  const dataObjectConstants =
    dataObjectSource === 'hubspot'
      ? ['contact', 'deal']
      ...
      <>
          ...
          <Col span={12}>
            <div className={'flex justify-end'}>
              <Button
                size={'large'}
                disabled={loading}
                onClick={() => setShowSmartEventForm(false)}
              >
                Cancel
              </Button>
            </div>
        ...
        <Row className={'mt-8'}>
          <Col span={18}>
            ...
            <Text type={'title'} level={6} extraClass={'m-0'} weight={'bold'}>
              {smartEventState?.expr?.description}
            </Text>
          </Col>
        </Row>
        ...
        <Row className={'mt-8'}>
          <Col span={18}>
            ...
            <Text type={'title'} level={6} extraClass={'m-0'} weight={'bold'}>
              {smartEventState?.expr?.event_type
                ? smartEventState?.expr?.event_type
                : 'CRM transition based event'}
            </Text>
          </Col>
        </Row>
          ...
          <Row className={'mt-8'}>
              ...
              <div className={'flex items-center'}>
                ...
                <div>
                  <Text
                    ...
                    weight={'bold'}
                  >
                    {smartEventState?.expr?.filters[0]?.Rules[0]?.op}
                  </Text>
                </div>
                <div className={'ml-2'}>
                  <Text
                    ...
                    weight={'bold'}
                  >
                    {smartEventState?.expr?.filters[0]?.Rules[0]?.value}
                  </Text>
                </div>
                <Text type={'title'} level={7} extraClass={'m-0 mx-2'}>
                  and previous value
                </Text>
                <div>
                  <Text
                    ...
                    weight={'bold'}
                  >
                    {smartEventState?.expr?.filters[0]?.Rules[1]?.op}
                  </Text>
                </div>
                <div className={'ml-2'}>
                  <Text
                    ...
                    weight={'bold'}
                  >
                    {smartEventState?.expr?.filters[0]?.Rules[1]?.value}
                  </Text>
                </div>
              </div>
            </Col>
            <Col span={24}>
              <Text
                ...
                extraClass={'m-0 mt-2'}
              >
                Previous and current values of the object property will be
                ...
                <>
                  ...
                  <Row className={'mt-8'}>
                    <Col span={18}>
                      ...
                      <Form.Item name='event_type' className={'m-0'}>
                        <Select
                  ...
                  <Row className={'mt-8'}>
                    <Col span={18}>
                      ...
                      >
                        <Select
                          className={'fa-select w-full'}
                          onChange={(value) => setDataObjectSource(value)}
                      ...
                      <Col span={18}>
                        ...
                        <Form.Item
                          ...
                          rules={[
                            {
                              required: true,
                              message: 'Please select a data object.'
                            }
                          ]}
                        >
                          <Select
                            onChange={(value) => {
                              onSelectDataObjectChange(value);
                            }}
                            className={'fa-select w-full'}
                          ...
                          rules={[
                            {
                              required: true,
                              message:
                                'Please select a property evaluation type.'
                            }
                          ]}
                        >
                          <Radio.Group
                            onChange={(e) => onChangeRuleforEvaluation(e)}
                          >
                            <Radio value={'any'}>Any change in property</Radio>
                            <Radio value={'specific'}>
                              Specific change in property
                            </Radio>
                          </Radio.Group>
                        </Form.Item>
                      </Col>
                        ...
                        <Col span={18}>
                          ...
                          <Form.Item
                            ...
                            rules={[
                              {
                                required: true,
                                message: 'Please select an object property.'
                              }
                            ]}
                          >
                            <Select
                    ...
                    <Row className={'mt-8'}>
                      <Col span={24}>
                        <div className={'flex items-center'}>
                          ...
                          <div className={'ml-2'}>
                            <Select
                              defaultValue={specificPropertiesData?.[0]}
                              value={currVal}
                              onChange={(value) => setCurrVal(value)}
                              className={'fa-select w-full ml-2'}
                          ...
                          <div className={'ml-2'}>
                            <Select
                              defaultValue={specificPropertiesData?.[0]}
                              value={lastVal}
                              onChange={(value) => setLastVal(value)}
                              className={'fa-select w-full'}
                              placeholder={'Object Property'}
                            >
                              {specificPropertiesData?.map((item) => {
                      ...
                      <Col span={24}>
                        <Text
                          ...
                          extraClass={'m-0 mt-2'}
                        >
                          Previous and current values of the object property
                          will be available as custom event properties
                        </Text>
                      </Col>
                    </Row>
                  )}
                      ...
                      <Col span={18}>
                        ...
                        <Form.Item
                          ...
                          rules={[
                            {
                              required: true,
                              message: 'Please select a time of event'
                            }
                          ]}
                        >
                          <Radio.Group
                            onChange={(value) => settimestampReference(value)}
                          >
                            <Radio value={'timestamp_in_track'}>
                      ...
                      <Col span={18}>
                        <Form.Item
                          ...
                          rules={[
                            {
                              required: true,
                              message: 'Please select a date type property.'
                            }
                          ]}
                        >
                          <Select
                            className={'fa-select w-full mt-2'}
                            placeholder={'List all the date type proprties  '}
                            size={'large'}
                            showSearch
                            optionFilterProp='children'
                            filterOption={(input, option) =>
