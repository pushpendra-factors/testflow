...
function CoreQuery({ setDrawerVisible, setQueryType, setQueries, setRowClicked, setQueryOptions, location }) {
  ...
  const setQueryToState = useCallback((record) => {
    ...
    setQueryOptions(currData => {
      return {
        ...currData,
        groupBy: [...equivalentQuery.breakdown.global, ...equivalentQuery.breakdown.event]
      }
    });
  ...
  const getMenu = (row) => {
    return (
      <Menu>
        <Menu.Item key="0">
          <a onClick={handleViewResult.bind(this, row)} href="#!">View Results</a>
        </Menu.Item>
        <Menu.Item key="1">
          <a onClick={(e) => e.stopPropagation()} href="#!">Copy Link</a>
        </Menu.Item>
        <Menu.Item key="2">
          <a onClick={handleDelete.bind(this, row)} href="#!">Delete Query</a>
        </Menu.Item>
      </Menu>
    )
  };
