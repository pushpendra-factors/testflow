...
export const getHostUrl = () => {
  ...
  let isSlothApp =
    ...
    window.location.host.indexOf('sloth') == 0;
  // TODO: Remove isFlashApp after deployment of sloth on production.
  let isFlashApp =
    window.location &&
    window.location.host &&
    window.location.host.indexOf('flash') == 0;
...
function request(dispatch, method, url, headers, data) {
  ...
  if (
    window.USE_FILTER_OPT_EVENTS_USERS &&
    window.USE_FILTER_OPT_EVENTS_USERS != ''
  ) {
  ...
  return fetch(url, options).then((response) => {
    ...
    return response.text().then((text) => {
      const responsePayload = {
        ...
        ok: isRequestSuccess(response.status)
      };
...
export function get(dispatch, url, headers = {}) {
  return request(dispatch, 'GET', url, headers);
}

export function post(dispatch, url, data, headers = {}) {
  return request(dispatch, 'POST', url, headers, data);
}

export function put(dispatch, url, data, headers = {}) {
  return request(dispatch, 'PUT', url, headers, data);
}

export function del(dispatch, url, data, headers = {}) {
  return request(dispatch, 'DELETE', url, headers, data);
}

export function patch(dispatch, url, data, headers = {}) {
  return request(dispatch, 'PATCH', url, headers, data);
}
