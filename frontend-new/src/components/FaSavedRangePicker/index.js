...
import { DatePicker, Menu, Dropdown, Button, Tooltip, Tag } from 'antd';
...
const FaDatepicker = ({
  ...
  const onSavedRangeSelected = (data) => {
    onSelectSavedRange(data);
    const newDateData = {
      ...dateData,
  ...
  const savedDateRanges = () => {
    if (savedRanges) {
      return savedRanges?.map((item, index) => {
        let dateString = `${moment.unix(item?.startDate).format('MMM DD, YYYY')} - ${moment.unix(item?.endDate).format('MMM DD, YYYY')}`;
        let isDisabledState = item?.status == 'saved' || item?.status == 'building';
        return (
          <Menu.Item key={index} className={isDisabledState ? 'cursor-not-allowed' : ''} disabled={isDisabledState}>
            <a target="_blank" className='flex items-center justify-between' disabled={isDisabledState} onClick={() => onSavedRangeSelected(item)}>
              {dateString}
              {isDisabledState ? <span className='ml-2'><Tag className={'fa-tag--regular truncate'}>Processing..</Tag></span> : ""}
            </a>
          </Menu.Item>
        )
          ...
          <a target="_blank" onClick={e => e.preventDefault()} >
            <div className={"flex items-center justify-between"}>
              <div className={"flex items-center"}>
                <SVG name={'Plus'} size={18} color={'grey'} extraClass={'m-0 mr-1'} />
                Select New Range
              </div>
              <SVG name={'Arrowright'} size={18} color={'grey'} />
            </div>
          </a>
        </Dropdown>
      </Menu.Item>
    ...
    <div className="fa-custom-datepicker">
      {
        ...
        >
          ...
          {!showDatePicker && !range ? 'Select date range' : null}
          {showDatePicker && (
            <RangePicker
              format="MMM DD YYYY"
  ...
  const renderFaDatePicker = () => (
    <div className="fa-custom-datepicker">
      <Dropdown
        disabled={disabled}
        overlayClassName="fa-custom-datepicker--parent"
        overlay={savedRanges ? parentMenu : menu}
        placement={placement}
