...
function ChartLegends({ events, colors, eventsMapper, focusHoveredLines, focusAllLines, setHiddenEvents, hiddenEvents }) {
    ...
    return (
        <div className="flex flex-wrap items-center justify-center w-full">
            {events.map((event, index) => {
                return (
                    <div onClick={handleLegendClick.bind(this, event)} onMouseOver={focusHoveredLines.bind(this, eventsMapper[event])} onMouseOut={focusAllLines} key={event + index} className={`${hiddenEvents.indexOf(event) > -1 ? 'opacity-25' : 'opacity-100'} flex items-center cursor-pointer`}>
                        <div style={{
