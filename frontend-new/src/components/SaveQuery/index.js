...
function SaveQuery({
  ...
  const toggleAddToDashboard = useCallback((val) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (addToDashboard) {
        ...
        await saveQueryToDashboard(active_project.id, selectedDashboards.join(','), reqBody);
      }
          ...
          >
            {
              dashboards.data.map(d => {
                return (
                  <Select.Option value={d.name} key={d.id}>{d.name}</Select.Option>
                );
              })
            }
          </Select>
    ...
    <>
      ...
      >
        <div className="p-4">
          ...
          <Text extraClass={`pt-1 ${styles.noteText}`} mini type={'paragraph'}>{dashboardHelpText}</Text>
          {dashboardOptions}
        </div>
      </Modal>
    </>
