...
function SearchBar({ searchText, handleSearchTextChange, searchBar }) {
  ...
  const downloadBtn = (
        <div onClick={downloadCSV} className="flex flex-1 items-center justify-end cursor-pointer">
            <Button size={'large'} style={{ display: 'flex', alignItems: 'center' }} icon={<SVG name={'download'} size={24} color={'grey'} />} type="text">Download CSV</Button>
        </div>
  );
            ...
            {!searchBar ? (
                <div className="flex pb-1 w-full">
                    ...
                    <Input
                        ...
                        value={searchText}
                        className={`${styles.inputSearchBar} ${!searchText.length ? styles.inputPlaceHolderFont : styles.inputTextFont}`}
                        size="large"
                        placeholder="Search"
                        prefix={(
                            <svg width="17" height="18" viewBox="0 0 17 18" fill="none" xmlns="http://www.w3.org/2000/svg">
