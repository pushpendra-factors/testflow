...
const FaSelect = ({ options, delOption, 
    ...
    applClick,
    multiSelect=false,
    onClickOutside, selectedOpts = [],  allowSearch = false, posRight=false }) => {
    ...
    useEffect(() => {
        if(multiSelect && selectedOpts && selectedOpts.length) {
            const arr = selectedOpts.map((op) => {return JSON.stringify([op])})
            setOptClickArr(arr);
        }
    ...
    const renderOptions = () => {
        ...
        if(searchTerm?.length) {
            options.forEach((op, index) => {
                ...
                if (op[0].toLowerCase().includes(searchTerm.toLowerCase())) {
                    rendOpts.push(
                        <div key={index} title={op[0]} className={`fa-select-group-select--options ${isSelected? styles.fa_selected : null}`}
                            onClick={() => optClick(() => optionClick(op), op)} >
                            {op[1] && !multiSelect? <SVG name={op[1]} extraClass={'self-center'}></SVG> : null}
                            <span className={`ml-1 ${styles.optText}`}>{op[0]}</span>
                            {isSelected? <SVG name="checkmark" extraClass={'self-center'} size={17} color={'purple'} /> : null}
                        </div>
                    )
                }
            });
            ...
            rendOpts.push(
                <div key={rendOpts.length + 2} className={`fa-select-group-select--options ${isSelectedCheck([searchTerm])? styles.fa_selected : null}`}
                    onClick={() => optClick(() => optionClick([searchTerm]), [searchTerm])} >
                    <Text level={7} type={'title'} extraClass={'m-0'} weight={'thin'}> Select: </Text>
        ...
        if(multiSelect) {
            if(rendOpts.length >= 4) {
                rendOpts.push(
                    <div key={101} className={`fa-select-group-select--options ${styles.dropdown__empty_opt}`}
                         >
                    </div>
                )
            }
            rendOpts.push(
                <div key={100} className={`fa-select-group-select--options ${styles.dropdown__apply_opt}`}
                    onClick={applyClick} >
                    <Button disabled={!optClickArr.length} type="primary">Apply</Button>
                </div>
            )
        }
    ...
    const renderSearchInput = () => {
        return (
            <div className={`${styles.selectInput} fa-filter-select`}>
                <Input prefix={<SVG name={"search"} />} size="large" placeholder={"Search"} onChange={search}></Input>
            </div>
        )
    }
    ...
    return (<>
            <div className={`${posRight? styles.dropdown__select_rt : styles.dropdown__select} fa-select ${posRight? `fa-select--group-select-sm` : `fa-select--group-select`}`}>
