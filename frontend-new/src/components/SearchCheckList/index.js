...
export default function SearchCheckList({
  ...
  const [sortableList, setSortableList] = useState(
    getListWithDisabledOptions(getUniqueItemsByKeyAndSearchTerm(mapArray, searchTerm))
  );
  ...
  useEffect(() => {
    setSortableList(getListWithDisabledOptions(getUniqueItemsByKeyAndSearchTerm(mapArray, searchTerm)));
  }, [mapArray, searchTerm]);
        ...
        <div className={`${showApply ? 'apply_active' : ''}`}>
          {mapArray?.length ? (
            <ReactSortable
                    ...
                    style={{ height: showApply ? '348px' : '392px', overflowY: 'auto' }}
                    filter=".not-draggable"
                    {...sortableOptions}
            >
            {sortableList?.map((item, index) => (
              <div key={item[titleKey]} className={ (item?.isDisabled) || !(item[checkedKey]) ? 'not-draggable' : ''}>
                {item?.isDisabled ? (
                  <div
                    className='flex justify-between items-center py-2 px-3 cursor-not-allowed'
                    onClick={(option) => handleDisableOptionClick && handleDisableOptionClick(option)}
                  >
                    <Text type='title' level={7} extraClass='mb-0 truncate' truncate charLimit={25}>
                      {item?.name}
                    </Text>
