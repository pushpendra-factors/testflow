...
import { Layout, Button, Avatar, Popover, Modal, Row, Col } from 'antd';
    ...
    return (
      <div data-tour='step-9' className={'fa-popupcard'}>
        ...
        >
          ...
          >{`${props.currentAgent?.first_name?.charAt(0)}${props.currentAgent?.last_name?.charAt(
            0
          )}`}</Avatar>
  ...
  return (
    <>
      <Popover
        ...
        trigger='click'
      >
        <Button className={`${styles.button} flex items-center mr-4`} type='text' size='large'>
          <Avatar
            size={36}
            shape='square'
            style={{
          ...
          <div className='flex flex-col items-start ml-2'>
            <div className='flex items-center'>
              <Text type={'title'} level={7} extraClass={'m-0'} weight={'bold'}>
                {`${props.currentAgent?.first_name} ${props.currentAgent?.last_name}`}
              </Text>
          ...
          <Row>
            <Col span={24}>
              ...
              <Text type={'title'} level={7} color={'grey'} extraClass={'m-0 mt-2'}>
                You can easily switch between projects. You will be redirected a different dataset.
              </Text>
            </Col>
          </Row>
        </div>
