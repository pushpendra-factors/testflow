...
if [ "$RECOPY_DB_DATA" = true ]; then
    ...
    mysqldump  -u $DB_USER_NAME -h $DB_HOST -p$DB_PASSWORD factors account_scoring_ranges ads_import adwords_documents alerts bigquery_settings clickable_elements content_groups crm_activities crm_groups crm_properties crm_relationships crm_settings crm_users custom_metrics --where="project_id=$PROJECT_ID" | mysql -h $VM_HOST --port 3306 -u root -pdbfactors123 -D factors

    mysqldump  -u $DB_USER_NAME -h $DB_HOST -p$DB_PASSWORD factors dash_query_results dashboard_units dashboards data_availabilities display_name_labels display_names event_names event_properties_json explain_v2 facebook_documents feature_gates feedbacks fivetran_mappings form_fills g2_documents --where="project_id=$PROJECT_ID" | mysql -h $VM_HOST --port 3306 -u root -pdbfactors123 -D factors

    mysqldump  -u $DB_USER_NAME -h $DB_HOST -p$DB_PASSWORD factors group_relationships groups integration_documents leadgen_settings leadsquared_markers linkedin_documents otp_rules pathanalysis project_agent_mappings project_billing_account_mappings project_model_metadata project_plan_mappings --where="project_id=$PROJECT_ID" | mysql -h $VM_HOST --port 3306 -u root -pdbfactors123 -D factors

    mysqldump  -u $DB_USER_NAME -h $DB_HOST -p$DB_PASSWORD factors project_settings property_details property_mappings property_overrides queries salesforce_documents scheduled_tasks segments shareable_url_audits shareable_urls smart_properties smart_property_rules task_execution_details templates  --where="project_id=$PROJECT_ID" | mysql -h $VM_HOST --port 3306 -u root -pdbfactors123 -D factors

    mysqldump  -u $DB_USER_NAME -h $DB_HOST -p$DB_PASSWORD factors upload_filter_files user_properties_json website_aggregation weekly_insights_metadata --where="project_id=$PROJECT_ID" | mysql -h $VM_HOST --port 3306 -u root -pdbfactors123 -D factors
