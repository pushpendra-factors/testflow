...
func AttributionHandler(c *gin.Context) (interface{}, int, string, string, bool) {
	...
	if !hardRefresh && isDashboardQueryRequest && !H.ShouldAllowHardRefresh(requestPayload.Query.From, requestPayload.Query.To, timezoneString, hardRefresh) {
		...
		shouldReturn, resCode, resMsg := H.GetResponseIfCachedDashboardQuery(reqId, projectId, dashboardId, unitId, preset, effectiveFrom, effectiveTo, timezoneString)
		if shouldReturn {
	...
	if isDashboardQueryRequest {
		model.SetCacheResultByDashboardIdAndUnitId(result, projectId, dashboardId, unitId, preset,
