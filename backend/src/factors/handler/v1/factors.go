...
func PostFactorsCompareHandler(c *gin.Context) {
	...
	if comparisonMode == "Week" {
		...
		if results1, err, _ = PW.FactorV1("",
			projectId, params.StartEvent, startConstraints,
			params.EndEvent, endConstraints, P.COUNT_TYPE_PER_USER, ps1, "", nil, in_en, in_epr, in_upr); err != nil {
			logCtx.WithError(err).Error("Factors failed.")
			if err.Error() == "root node not found or frequency 0" {
				c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": "No Insights Found"})
			}
		...
		if results2, err, _ = PW.FactorV1("",
			projectId, params.StartEvent, startConstraints,
			params.EndEvent, endConstraints, P.COUNT_TYPE_PER_USER, ps2, "", nil, in_en, in_epr, in_upr); err != nil {
			logCtx.WithError(err).Error("Factors failed.")
			if err.Error() == "root node not found or frequency 0" {
				c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": "No Insights Found"})
			}
	...
	if comparisonMode == "Property" {
		...
		params1, in_en1, in_epr1, in_upr1 := MapRule(ipParams.Rule[0])
		params2, in_en2, in_epr2, in_upr2 := MapRule(ipParams.Rule[1])
		ps, err := PW.NewPatternServiceWrapper("", projectId, modelId)
		if err != nil {
			logCtx.WithError(err).Error("Pattern Service initialization failed.")
			c.JSON(http.StatusBadRequest, gin.H{
		...
		if results1, err, _ = PW.FactorV1("",
			...
			params1.EndEvent, endConstraints, P.COUNT_TYPE_PER_USER, ps, "", nil, in_en1, in_epr1, in_upr1); err != nil {
			logCtx.WithError(err).Error("Factors failed.")
			if err.Error() == "root node not found or frequency 0" {
				c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": "No Insights Found"})
			}
		...
		if results2, err, _ = PW.FactorV1("",
			...
			params2.EndEvent, endConstraints, P.COUNT_TYPE_PER_USER, ps, "", nil, in_en2, in_epr2, in_upr2); err != nil {
			logCtx.WithError(err).Error("Factors failed.")
			if err.Error() == "root node not found or frequency 0" {
				c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": "No Insights Found"})
			}
...
func GetModelMetaData(c *gin.Context) {
	...
	metadata, err := F.GetChunksMetaData(projectId, modelId)
	if err != nil {
		logCtx.WithError(err).Error("failed to retrieve metadata")
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
