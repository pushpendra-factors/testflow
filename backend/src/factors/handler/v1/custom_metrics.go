...
func DeleteCustomMetrics(c *gin.Context) (interface{}, int, string, string, bool) {
	...
	kpiInConfig, statusCode2 := store.GetStore().GetAttributionSettingsKPIListForCustomKPI(projectID, customMetric.Name)
	log.WithFields(log.Fields{"kpiInConfig": kpiInConfig, "statusCode": statusCode2}).Warn("Attribution Config impacted")

	if len(dashboardUnitNames) == 0 && len(alertNames) == 0 && len(derivedKPINames) == 0 && len(kpiInConfig) == 0 {
		...
		log.WithFields(log.Fields{"dashboardUnitNames": dashboardUnitNames1, "alertNames": alertNames,
			"derivedKPINames": derivedKPINames, "kpiInConfig": kpiInConfig}).Warn("All alerts including Dashboards impacted")
		errorMessage := BuildDependentsErrorMessage("Custom KPI",
			[][]string{dashboardUnitNames, alertNames, derivedKPINames, kpiInConfig}, []string{"dashboard unit", "alert", "derived KPI", "AttributionConfig"})
