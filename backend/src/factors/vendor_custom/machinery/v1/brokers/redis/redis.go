...
import (
	...
	"factors/vendor_custom/machinery/v1/brokers/errs"
...
func (b *Broker) Publish(ctx context.Context, signature *tasks.Signature) error {
	...
	if signature.ETA != nil {
		...
		// When ETA is met and RetryLaterOnPriority is true,
		// uses LPUSH and add task to top of queue.
		if signature.RetryLaterOnPriority {
			log.WARNING.Printf("Retrying delayed task %s on priority", signature.UUID)
...
func (b *Broker) GetQueueLength(queue string) (int, error) {
	...
	return redis.Int(dataBytes, err)
}
...
func (b *Broker) GetDelayedTasksCount() (int, error) {
	...
	dataBytes, err := conn.Do("ZCOUNT", redisDelayedTasksKey, "-inf", "+inf")
	if err != nil {
		return 0, err
	}
...
func (b *Broker) consumeOne(delivery []byte, taskProcessor iface.TaskProcessor) error {
	...
	log.Debug(b.GetConfig().Debug, "Received new message: %s", delivery)
