	...
	if !*syncOnly {
		...
		for bi := range batches {
			...
			for pi := range batch {
				...
				wg.Add(1)
				go syncWorker(batch[pi], &wg, workerIndex, *numDocRoutines, &enrichStatus, allowedSalesforceProjectSettings[batch[pi]], *enrichPullLimit, enrichProcessLimit, *documentLookbackDays)
				workerIndex++
			}
			wg.Wait()
		}
