...
func TestGetAuthData(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		retEmail, _, err := helpers.ParseAndDecryptProtectedFields(key, 0, 0, ad.ProtectedFields)
		assert.Equal(t, email, retEmail)
	})
	t.Run("SuccessWithLogoutInThePast", func(t *testing.T) {
		dur := helpers.CookieExpiry * time.Second
		authStr, err := helpers.GetAuthData(email, agentUUID, key, dur)
		assert.Nil(t, err)
		assert.NotEmpty(t, authStr)
		ad, err := helpers.ParseAuthData(authStr)
		assert.Nil(t, err)
		assert.Equal(t, agentUUID, ad.AgentUUID)
		retEmail, _, err := helpers.ParseAndDecryptProtectedFields(key, time.Now().UTC().Unix()-100, time.Now().UTC().Unix()-100, ad.ProtectedFields)
		assert.Equal(t, email, retEmail)
	})
	...
	t.Run("ExpiredData", func(t *testing.T) {
		...
		_, errMsg, err = helpers.ParseAndDecryptProtectedFields(key, 0, 0, ad.ProtectedFields)
	...
	t.Run("LoggedoutUserPastCookieAccess", func(t *testing.T) {
		...
		assert.Nil(t, err)
		ad, err := helpers.ParseAuthData(authStr)
		_, errMsg, err = helpers.ParseAndDecryptProtectedFields(key, time.Now().UTC().Add(100*time.Second).Unix(), 0, ad.ProtectedFields)
	...
	t.Run("PasswordChangedUserPastCookieAccesss", func(t *testing.T) {
		...
		assert.Nil(t, err)
		ad, err := helpers.ParseAuthData(authStr)
		_, errMsg, err = helpers.ParseAndDecryptProtectedFields(key, 0, time.Now().UTC().Add(100*time.Second).Unix(), ad.ProtectedFields)
		assert.Equal(t, errMsg, "CookieInvalid")
		assert.NotNil(t, err)
	})
}
