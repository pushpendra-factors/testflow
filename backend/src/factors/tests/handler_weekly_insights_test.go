...
func TestWeeklyInsights(t *testing.T) {
	...
	assert.Nil(t, err)
	response, err := delta.GetWeeklyInsights(data.ProjectID, "", data.QueryId, &data.BaseStartTime, &data.CompStartTime, data.InsightsType, data.NumberOfRecords, 0, 0)
	...
	response, err = delta.GetWeeklyInsights(data.ProjectID, feedbackObj.CreatedBy, data.QueryId, &data.BaseStartTime, &data.CompStartTime, data.InsightsType, data.NumberOfRecords, 0, 0) // testing again after blacklisting
	assert.NotNil(t, response)
