...
func TestPageCountAndPageTimeSpentProperties(t *testing.T) {
	...
	Event1.SessionId = &firstSession.ID
	Event1.Timestamp = firstSession.Timestamp
	_, errCode = M.CreateEvent(Event1)
	assert.Equal(t, http.StatusCreated, errCode)
	...
	event2, errCode := M.CreateEvent(Event2)
	assert.Equal(t, http.StatusCreated, errCode)
	...
	Event3 := &M.Event{
		...
		UserId:           userId,
		UserPropertiesId: userPropertiesId,
		Timestamp:        secondSession.Timestamp + 10,
	}
	Event3.SessionId = &secondSession.ID
	event3, errCode := M.CreateEvent(Event3)
	assert.Equal(t, http.StatusCreated, errCode)
	// assert.True(t, event2.Timestamp < event.Timestamp)
	...
	assert.Nil(t, err)
	assert.Equal(t, (*firstSessionEventPropertiesMap)[U.EP_PAGE_COUNT], float64(2))
	assert.Equal(t, float64(event2.Timestamp-firstSession.Timestamp), (*firstSessionEventPropertiesMap)[U.EP_SESSION_TIME_SPENT])
	...
	assert.Nil(t, err)
	assert.Equal(t, float64(2), (*userProperties)[U.UP_PAGES_COUNT])
	assert.Equal(t, (*firstSessionEventPropertiesMap)[U.EP_SESSION_TIME_SPENT], (*userProperties)[U.UP_TOTAL_SESSIONS_TIME])

	thirdSession, errCode := M.CreateOrGetSessionEvent(projectId, userId, false, false, time.Now().Unix()+86400*2,
		&U.PropertiesMap{U.EP_PAGE_LOAD_TIME: 0.10}, &U.PropertiesMap{}, userPropertiesId)
	assert.Equal(t, http.StatusCreated, errCode)
	assert.NotNil(t, thirdSession)

	secondSessionEvent, errCode := M.GetEvent(projectId, userId, secondSession.ID)
	assert.Equal(t, http.StatusFound, errCode)
	secondSessionEventPropertiesMap, err := U.DecodePostgresJsonb(&secondSessionEvent.Properties)
	assert.Nil(t, err)
	assert.Equal(t, (*secondSessionEventPropertiesMap)[U.EP_PAGE_COUNT], float64(1))
	assert.Equal(t, float64(event3.Timestamp-secondSession.Timestamp), (*secondSessionEventPropertiesMap)[U.EP_SESSION_TIME_SPENT])
