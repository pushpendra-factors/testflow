...
func TestDeleteSession(t *testing.T) {
	...
	sessionEventName, status := store.GetStore().CreateOrGetEventName(&model.EventName{ProjectId: project.ID, Name: U.EVENT_NAME_SESSION, Type: model.TYPE_INTERNAL_EVENT_NAME})
	assert.Equal(t, http.StatusCreated, status)

	timestamp := time.Now().Unix()

	event := &model.Event{EventNameId: sessionEventName.ID, ProjectId: project.ID,
		UserId: user.ID, Timestamp: timestamp}
	sessionEvent1, errCode := store.GetStore().CreateEvent(event)

	event = &model.Event{EventNameId: sessionEventName.ID, ProjectId: project.ID,
		UserId: user.ID, Timestamp: timestamp + 1000}
	sessionEvent2, errCode := store.GetStore().CreateEvent(event)

	eventName, status := store.GetStore().CreateOrGetEventName(&model.EventName{ProjectId: project.ID, Name: getRandomName(), Type: model.TYPE_USER_CREATED_EVENT_NAME})
	assert.Equal(t, http.StatusCreated, status)

	event1 := &model.Event{EventNameId: eventName.ID, ProjectId: project.ID,
		UserId: user.ID, Timestamp: timestamp, SessionId: &sessionEvent1.ID}
	_, errCode = store.GetStore().CreateEvent(event1)
	assert.Equal(t, http.StatusCreated, errCode)

	event2 := &model.Event{EventNameId: eventName.ID, ProjectId: project.ID,
		UserId: user.ID, Timestamp: timestamp + 1000, SessionId: &sessionEvent2.ID}
	_, errCode = store.GetStore().CreateEvent(event2)
	assert.Equal(t, http.StatusCreated, errCode)

	events, status := store.GetStore().GetEventsByEventNameIDANDTimeRange(project.ID, sessionEventName.ID, timestamp, timestamp+1000)
	assert.Equal(t, http.StatusFound, status)
	assert.Len(t, events, 2)

	session1Events, status := store.GetStore().GetEventsBySessionEvent(project.ID, events[0].ID, events[0].UserId)
	assert.Equal(t, http.StatusFound, status)
	assert.Len(t, session1Events, 1)
	session2Events, status := store.GetStore().GetEventsBySessionEvent(project.ID, events[1].ID, events[1].UserId)
	assert.Equal(t, http.StatusFound, status)
	assert.Len(t, session2Events, 1)

	status = store.GetStore().DissociateEventsFromSession(project.ID, []model.Event{*event1}, sessionEvent1.ID)
	assert.Equal(t, http.StatusAccepted, status)
	event, status = store.GetStore().GetEventById(project.ID, event1.ID, event1.UserId)
	assert.Equal(t, http.StatusFound, status)
	assert.Nil(t, event.SessionId)

	event, status = store.GetStore().GetEventById(project.ID, event2.ID, event2.UserId)
	assert.Equal(t, http.StatusFound, status)
	assert.Equal(t, sessionEvent2.ID, *event.SessionId)
}
