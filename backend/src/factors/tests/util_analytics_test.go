...
func TestGetAllWeeksAsTimestamp(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := U.GetAllWeeksAsTimestamp(tt.args.fromUnix, tt.args.toUnix, tt.args.timezone, false); !reflect.DeepEqual(got, tt.want) {
				for i, x := range got {
...
func TestGetAllMonthsAsTimestamp(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := U.GetAllMonthsAsTimestamp(tt.args.fromUnix, tt.args.toUnix, tt.args.timezone, false); !reflect.DeepEqual(got, tt.want) {
				for i, x := range got {
					if x.Unix() != tt.want[i].Unix() {
						t.Errorf("GetAllWeeksAsTimestamp() = %d, want %d", x.Unix(), tt.want[i].Unix())
					}
