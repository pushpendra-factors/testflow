...
func sendGetDashboardUnitChannelV1Result(r *gin.Engine, projectId uint64, agent *model.Agent, dashboardId uint64, dashboardUnitId uint64, query *model.ChannelGroupQueryV1) *httptest.ResponseRecorder {
	cookieData, err := helpers.GetAuthData(agent.Email, agent.UUID, agent.Salt, 100*time.Second)
	if err != nil {
		log.WithError(err).Error("Error creating cookie data.")
	}
