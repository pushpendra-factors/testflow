...
func TestHubspotContactList(t *testing.T) {
	project, _, err := SetupProjectWithAgentDAO()
	assert.Nil(t, err)
	r := gin.Default()
	H.InitDataServiceRoutes(r)
	...
	for i := 1; i <= 5; i++ {
		...
		status := store.GetStore().CreateHubspotDocument(project.ID, &hubspotContactDocument)
		assert.Equal(t, http.StatusCreated, status)
	}

	// Duplicate record
	enJson, err := json.Marshal(contacts[2])
	assert.Nil(t, err)
	...
	docs, status := store.GetStore().GetHubspotDocumentByTypeAndActions(project.ID, []string{"1", "2", "3", "4", "5"}, model.HubspotDocumentTypeContact, []int{model.HubspotDocumentActionCreated})
	assert.Equal(t, http.StatusFound, status)
	...
	contactList := map[string]interface{}{
		...
		"internalListId": 1,
		"deleteable":     true,
		"listMemberships": map[string][]map[string]interface{}{
			"1": {
				{
					"static-list-id":   1,
					"internal-list-id": 1,
				...
				},
				{
					"static-list-id":   2,
					"internal-list-id": 2,
					"timestamp":        time.Now().Add(-3*time.Minute).UnixNano() / int64(time.Millisecond),
					"vid":              1,
					"is-member":        true,
				},
				{
			...
			"2": {
				{
					"static-list-id":   1,
					"internal-list-id": 1,
					"timestamp":        time.Now().Add(-4*time.Minute).UnixNano() / int64(time.Millisecond),
					"vid":              2,
					"is-member":        true,
				},
				{
	...
	docs, status = store.GetStore().GetHubspotDocumentByTypeAndActions(project.ID, []string{"1"}, model.HubspotDocumentTypeContactList, []int{model.HubspotDocumentActionCreated})
	assert.Equal(t, http.StatusFound, status)
		...
		"listMemberships": map[string][]map[string]interface{}{
			...
			"5": {
				...
				},
				{
					"static-list-id":   5,
					"internal-list-id": 10,
					"timestamp":        time.Now().Add(-1*time.Second).UnixNano() / int64(time.Millisecond),
					"vid":              5,
					"is-member":        true,
				},
			},
			"3": {
				{
					"static-list-id":   1,
					"internal-list-id": 1,
					"timestamp":        time.Now().Add(-30*time.Second).UnixNano() / int64(time.Millisecond),
					"vid":              3,
					"is-member":        true,
				},
			},
	...
	docs, status = store.GetStore().GetHubspotDocumentByTypeAndActions(project.ID, []string{"1"}, model.HubspotDocumentTypeContactList, []int{model.HubspotDocumentActionUpdated})
	assert.Equal(t, http.StatusFound, status)
	for _, document := range docs {
		assert.Equal(t, true, document.Synced)
	}
