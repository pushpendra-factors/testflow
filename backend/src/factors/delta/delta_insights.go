...
func ComputeDeltaInsights(projectId int64, configs map[string]interface{}) (map[string]interface{}, bool) {
	...
	for _, dashboardUnit := range dashboardUnits {
		...
		if len(kpiQuery.Queries) > 0 {
			...
			err := processSeparatePeriods(projectId, periodCodesWithWeekNMinus1, archiveCloudManager, tmpCloudManager, sortedCloudManager, modelCloudManager, diskManager, deltaQuery, multiStepQuery, k, &unionOfFeatures, 1, isEventOccurence, isMultiStep, skipWpi, skipWpi2, mailerRun, beamConfig, hardPull, useSortedFilesMerge, pulledMap)
			if err != nil {
			...
			err = processSeparatePeriods(projectId, periodCodesWithWeekNMinus1, archiveCloudManager, tmpCloudManager, sortedCloudManager, modelCloudManager, diskManager, deltaQuery, multiStepQuery, k, &unionOfFeatures, 2, isEventOccurence, isMultiStep, skipWpi, skipWpi2, mailerRun, beamConfig, hardPull, useSortedFilesMerge, pulledMap)
			if err != nil {
...
func processSeparatePeriods(projectId int64, periodCodes []Period, archiveCloudManager, tmpCloudManager, sortedCloudManager, modelCloudManager *filestore.FileManager, diskManager *serviceDisk.DiskDriver, deltaQuery Query, multiStepQuery MultiFunnelQuery, k int, unionOfFeatures *(map[string]map[string]bool), passId int, isEventOccurence bool, isMultiStep bool, skipWpi bool, skipWpi2 bool, mailerRun bool, beamConfig *merge.RunBeamConfig, hardPull, useSortedFilesMerge bool, pulledMap map[int64]map[string]bool) error {
	...
	for _, periodCode := range periodCodes {
		...
		if !skipWpi || (!earlierWeekMap[periodCode.From] && !skipWpi2) {
			err := processSinglePeriodData(projectId, periodCode, archiveCloudManager, tmpCloudManager, sortedCloudManager, modelCloudManager, diskManager, deltaQuery, multiStepQuery, k, unionOfFeatures, passId, isEventOccurence, isMultiStep, mailerRun, beamConfig, hardPull, useSortedFilesMerge, pulledMap)
			if err != nil {
				return err
			}
		} else {
...
func processSinglePeriodData(projectId int64, periodCode Period, archiveCloudManager, tmpCloudManager, sortedCloudManager, modelCloudManager *filestore.FileManager, diskManager *serviceDisk.DiskDriver, deltaQuery Query, multiStepQuery MultiFunnelQuery, k int, unionOfFeatures *(map[string]map[string]bool), passId int, isEventOccurence bool, isMultiStep bool, mailerRun bool, beamConfig *merge.RunBeamConfig, hardPull, useSortedFilesMerge bool, pulledMap map[int64]map[string]bool) error {
	scanner, err := GetEventFileScanner(projectId, periodCode, archiveCloudManager, tmpCloudManager, sortedCloudManager, diskManager, beamConfig, hardPull, useSortedFilesMerge, pulledMap)
	if err != nil {
