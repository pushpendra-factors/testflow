...
ALTER TABLE weekly_insights_metadata ADD CONSTRAINT weekly_insights_metadata_query_id_projects_id_foreign FOREIGN KEY (query_id) REFERENCES public.queries (id) MATCH SIMPLE
        ON UPDATE RESTRICT
        ON DELETE RESTRICT;
...
CREATE UNIQUE INDEX weekly_insights_metadata_project_id_stdate_enddate_unique_idx on weekly_insights_metadata(project_id, query_id, base_start_time, base_end_time, comparison_start_time, comparison_end_time);
