		...
		for _, p := range goalPatternsFromDB {
			if valPattern, ok := tmpPatterns[p.Name]; ok {
				...
				if _, ok := eventNamesWithType[p.Name]; ok {
					...
					mineLog.Info(fmt.Sprint("Goal event from db added from events in eventsFile ", p.String()))
...
func GetEventNamesFromFile(scanner *bufio.Scanner, cloudManager *filestore.FileManager, projectId uint64, modelId uint64, countsVersion int) ([]string, error) {
	...
	// 		for k, vc := range pMap {
	// 			for v, c := range vc {
	// 				propKey := strings.Join([]string{k, v}, "::")
	// 				x[propKey] = c
	// 			}
	// 		}
	// 		U.FilterOnFrequency(x, topk)
	// 		pMap = make(map[string]map[string]int)
	// 		for propKey, count := range x {
	// 			kv := strings.Split(propKey, "::")
	// 			k, v := kv[0], kv[1]
	// 			if _, ok := pMap[k]; !ok {
	// 				pMap[k] = make(map[string]int)
	// 			}
	// 			pMap[k][v] = count
	// 		}
	// 		userKeys += len(x)
