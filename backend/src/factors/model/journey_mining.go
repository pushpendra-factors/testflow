...
func getUserEventsFromFileChargebee(fileName string, segmentAGoals, segmentBGoals, segmentCGoals []QueryEventWithProperties,
...
// TODO(prateek): Add tests for this. Move to analytics.go model.
...
func areEqualEventsWithFilters(denormalizedEvent ArchiveEventFormat, journeyEvent QueryEventWithProperties) bool {
	...
	if journeyEvent.Name == denormalizedEventName {
		...
		for index, property := range journeyEvent.Properties {
			...
			if property.Entity == PropertyEntityEvent {
...
func satisfiesAnyJourneyEvent(denormalizedEvent ArchiveEventFormat,
	...
	return false, QueryEventWithProperties{}
}
...
func pruneUserEventsPath(userEventsMap *map[string][]ArchiveEventFormat,
	goalEvents []QueryEventWithProperties, journeyEvents []QueryEventWithProperties,
...
func dedupJourneyEvents(journeyEvents []QueryEventWithProperties) []QueryEventWithProperties {
	journeyEventsMap := make(map[string]bool)
	dedupedEventsList := make([]QueryEventWithProperties, 0, 0)
...
func createEventAliasLegendForJourneyEvents(journeyEvents []QueryEventWithProperties) (
...
func prettyPrintJourneyPatterns(projectID uint64, allPatternsMap map[string]*JourneyPatternMap, outputFileContents string,
	journeyEvents []QueryEventWithProperties, eventAliasLegend, reverseEventAliasLegend map[string]string) {
...
func GetWeightedJourneyMatrix(projectID uint64, journeyEvents []QueryEventWithProperties,
	...
	eventFiles, userFiles string, includeSession bool, sessionProperty string, cloudManager filestore.FileManager) {
	logCtx := log.WithFields(log.Fields{
	...
	if includeSession {
		...
		journeyEvents = append(
			[]QueryEventWithProperties{QueryEventWithProperties{Name: U.EVENT_NAME_SESSION}},
			journeyEvents...)
	}
...
func computeInsights5(userEventsMap map[string][]ArchiveEventFormat, segmentUsers map[string]bool) {
	trialSignupEvent := QueryEventWithProperties{Name: "www.chargebee.com/trial-signup"}
	scheduleADemoEvent := QueryEventWithProperties{Name: "www.chargebee.com/schedule-a-demo"}
...
func computeInsights4(userEventsMap map[string][]ArchiveEventFormat, segmentUsers map[string]bool, segmentVisitedPages []QueryEventWithProperties) {
...
func computeInsights3(userEventsMap map[string][]ArchiveEventFormat, segmentUsers map[string]bool) {
	...
	sessionEvent := QueryEventWithProperties{Name: U.EVENT_NAME_SESSION}
...
func ChargebeeAnalysis(segmentAGoals, segmentBGoals, segmentCGoals, segmentVisitedPages []QueryEventWithProperties,
	sourceFileNames string, startTime, endTime int64, cloudManager filestore.FileManager) {
	userEventsMap := make(map[string][]ArchiveEventFormat)
	...
	for _, fileName := range fileNames {
		...
		getUserEventsFromFileChargebee(fileName, segmentAGoals, segmentBGoals, segmentCGoals, &userEventsMap,
			&segmentAUsers, &segmentBUsers, &segmentCUsers, []QueryEventWithProperties{},
