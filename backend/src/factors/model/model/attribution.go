	...
	for _, row := range rows {
		...
		for i := keyIndex + 18; i < len(grandTotalRow); i += 3 {
			...
			if spend > 0 && i < len(grandTotalRow) && j < len(spendFunnelConversionCPC) {
				...
				spendFunnelConversionCPC[j] = 0
				conversionFunnelConversionCPC[j] = 0
			}
