...
func (store *MemSQL) GetSmartPropertiesRulesConfig(projectID uint64, objectType string) (model.SmartPropertiesRulesConfig, int) {
...
func (store *MemSQL) CreateSmartPropertiesRules(projectID uint64, smartPropertiesRulesDoc *model.SmartPropertiesRules) (string, int) {
	...
	errCode := store.checkIfRuleNameAlreadyPresentWhileCreate(projectID, smartPropertiesRulesDoc.Name, objectType)
...
func (store *MemSQL) UpdateSmartPropertiesRules(projectID uint64, ruleID string, smartPropertiesRulesDoc model.SmartPropertiesRules) (string, int) {
	logCtx := log.WithField("project_id", smartPropertiesRulesDoc.ProjectID)
	...
	errCode := store.checkIfRuleNameAlreadyPresentWhileUpdate(projectID, smartPropertiesRulesDoc.Name, ruleID, objectType)
	if errCode == http.StatusFound {
		return "Name already present.", http.StatusBadRequest
	}
...
func (store *MemSQL) GetSmartPropertiesRules(projectID uint64) ([]model.SmartPropertiesRules, int) {
	smartPropertiesRules := make([]model.SmartPropertiesRules, 0, 0)
	if projectID == 0 {
...
func (store *MemSQL) GetAllChangedSmartPropertiesRulesForProject(projectID uint64) ([]model.SmartPropertiesRules, int) {
	smartPropertiesRules := make([]model.SmartPropertiesRules, 0, 0)
	db := C.GetServices().Db
...
func (store *MemSQL) GetSmartPropertiesRule(projectID uint64, ruleID string) (model.SmartPropertiesRules, int) {
...
func (store *MemSQL) DeleteSmartPropertiesRules(projectID uint64, ruleID string) int {
	if projectID == 0 {
		log.Error("Invalid project ID.")
...
func (store *MemSQL) GetProjectIDsHavingSmartPropertiesRules() ([]uint64, int) {
	db := C.GetServices().Db
