...
func (store *MemSQL) GetPropertiesForMarketo(projectID int64, reqID string) []map[string]string {
	...
	marketoUsersOnlyProperties := model.TransformCRMPropertiesToKPIConfigProperties(properties, propertiesToDisplayNames, "$marketo")
	standardUserProperties := store.GetKPIConfigFromStandardUserProperties(projectID)
	return append(standardUserProperties, marketoUsersOnlyProperties...)
}
