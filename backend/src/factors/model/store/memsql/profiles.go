...
func (store *MemSQL) buildAllUsersQuery(projectID int64, query model.ProfileQuery) (string, []interface{}, error) {
	...
	if isGroupByTypeWithBuckets(query.GroupBys) {
		...
		bucketedStepName, aggregateSelectKeys, aggregateGroupBys, aggregateOrderBys := appendNumericalBucketingSteps(isAggregateOnProperty, &sqlStmnt, &params, query.GroupBys, "step_0", "", false, selectAliases, nil)
	...
	if projectID == 2 && query.SegmentID != "" && query.SegmentID == "191e3d76-d5b4-45cb-b106-58e834047727" {
		log.WithField("params", params).WithField("finalSQLStmnt", finalSQLStmnt).Warn("Query for debugging Segment level KPI")
	}
