...
func (store *MemSQL) CreatePropertyDetails(projectID uint64, eventName, propertyKey, propertyType string, isUserProperty bool) int {
	logCtx := log.WithFields(log.Fields{"project_id": projectID, "event_name": eventName, "property_key": propertyKey})
	...
	status, _ := store.GetPropertyTypeFromDB(projectID, eventName, propertyKey, isUserProperty)
