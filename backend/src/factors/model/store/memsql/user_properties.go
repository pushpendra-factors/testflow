...
func (store *MemSQL) createUserPropertiesIfChanged(projectID uint64, userId string,
	...
	if currentPropertiesId != "" {
		...
		currentPropertiesRecord, statusCode = store.GetUserPropertiesRecord(
	...
	if shouldMergeUserProperties {
		newPropertiesID, errCode := store.MergeUserPropertiesForUserID(projectID, userId,
...
func (store *MemSQL) BackFillUserDataInCacheFromDb(projectid uint64, currentDate time.Time,
	usersProcessedLimit int, propertiesLimit int, valuesLimit int, skipExpiryForCache bool) {
	...
	propertyCacheKey, err := model.GetUserPropertiesCategoryByProjectRollUpCacheKey(projectid, currentDateFormat)
	if err != nil {
	...
	begin := U.TimeNow()
	properties, err := store.GetRecentUserPropertyKeysWithLimits(projectid, usersProcessedLimit, propertiesLimit, currentDate)
	end := U.TimeNow()
	...
	for _, propertyValue := range properties {
		...
		begin := U.TimeNow()
		values, category, err := store.GetRecentUserPropertyValuesWithLimits(projectid, propertyValue.Key, usersProcessedLimit, valuesLimit, currentDate)
		end := U.TimeNow()
		...
		if category == U.PropertyTypeCategorical {
			PropertyValuesKey, err := model.GetValuesByUserPropertyRollUpCacheKey(projectid, propertyValue.Key, currentDateFormat)
			if err != nil {
...
func (store *MemSQL) MergeUserPropertiesForProjectID(projectID uint64, dryRun bool) int {
	logCtx := log.WithFields(log.Fields{
		"Method":    "MergeUserPropertiesForProjectID",
		"ProjectID": projectID,
	})

	customerUserIDs, errCode := store.GetDistinctCustomerUserIDSForProject(projectID)
	...
	for _, customerUserID := range customerUserIDs {
		...
		user, errCode := store.GetUserLatestByCustomerUserId(projectID, customerUserID)
		if errCode != http.StatusFound {
			logCtx.Infof("Error or no users found for customer user id")
			continue
		}
		_, errCode = store.MergeUserPropertiesForUserID(projectID, user.ID, postgres.Jsonb{}, "", U.TimeNowUnix(), dryRun, true)
		if errCode != http.StatusCreated && errCode != http.StatusNotModified {
...
func (store *MemSQL) MergeUserPropertiesForUserID(projectID uint64, userID string, updatedProperties postgres.Jsonb,
	...
	users, errCode := store.getUsersForMergingPropertiesByCustomerUserID(projectID, customerUserID)
	...
	var userPropertiesRecords []*model.UserProperties
	for _, user := range users {
		userPropertiesRecord, errCode := store.GetUserPropertiesRecord(projectID, user.ID, user.PropertiesId)
		if errCode != http.StatusFound {
	...
	userPropertiesRecords = append(userPropertiesRecords, &model.UserProperties{
		UserId:           userID,
		ProjectId:        projectID,
	...
	for i := range userPropertiesRecords {
		userPropertiesToBeMerged = append(userPropertiesToBeMerged, userPropertiesRecords[i].Properties)
	}
	...
	store.SanitizeAddTypeProperties(projectID, users, &mergedUserProperties)
...
func updateUserPropertiesForUser(projectID uint64, userID string, userProperties postgres.Jsonb,
	...
	userPropertiesRecord := model.UserProperties{
		ID:               U.GetUUID(),
		UserId:           userID,
		ProjectId:        projectID,
	...
	if updateUser {
		if err := db.Model(&model.User{}).Where("project_id = ? AND id = ?", projectID, userID).
...
func (store *MemSQL) SanitizeAddTypeProperties(projectID uint64, users []model.User, propertiesMap *map[string]interface{}) {
	var userIDs []string
	for _, user := range users {
	...
	sessionEvent, errCode := store.GetSessionEventName(projectID)
	if errCode != http.StatusFound {
		return
	}
	sessionCount, errCode := store.GetEventCountOfUsersByEventName(projectID, userIDs, sessionEvent.ID)
	if errCode != http.StatusFound {
		return
	}
...
func (store *MemSQL) GetUserProperties(projectId uint64, userId string, id string) (*postgres.Jsonb, int) {
	userPropertiesRecord, errCode := store.GetUserPropertiesRecord(projectId, userId, id)
...
func (store *MemSQL) GetUserPropertiesRecord(projectId uint64, userId string, id string) (*model.UserProperties, int) {
...
func (store *MemSQL) OverwriteUserProperties(projectId uint64, userId string,
	...
	db := C.GetServices().Db
	if err := db.Model(&model.UserProperties{}).
...
func (store *MemSQL) GetUserPropertiesRecordsByProperty(projectId uint64,
	key string, value interface{}) ([]model.UserProperties, int) {
...
func (store *MemSQL) UpdateUserPropertiesForSession(projectID uint64,
	...
	if C.ShouldUseUserPropertiesTableForRead(projectID) {
		return store.updateUserPropertiesForSessionV1(projectID, sessionUserPropertiesRecordMap)
	}

	return store.updateUserPropertiesForSessionV2(projectID, sessionUserPropertiesRecordMap)
}

func (store *MemSQL) updateUserPropertiesForSessionV1(projectID uint64,
	sessionUserPropertiesRecordMap *map[string]model.SessionUserProperties) int {
	...
	updatedUsersByUserID := make(map[string]*model.User, 0)
	...
	for userPropertiesID, sessionUserProperties := range *sessionUserPropertiesRecordMap {
		...
		userProperties, errCode := store.GetUserPropertiesRecord(projectID,
		...
		errCode = store.OverwriteUserProperties(projectID, sessionUserProperties.UserID,
		...
		if _, exists := updatedUsersByUserID[sessionUserProperties.UserID]; !exists {
			user, errCode := store.GetUser(projectID, sessionUserProperties.UserID)
			if errCode != http.StatusFound {
	...
	errCode := store.updateLatestUserPropertiesForSessionIfNotUpdatedV1(projectID,
...
func (store *MemSQL) updateLatestUserPropertiesForSessionIfNotUpdatedV1(projectID uint64,
	updatedUserPropertiesIDMap *map[string]bool, sessionUsersByUserID map[string]*model.User,
	...
	for _, user := range sessionUsersByUserID {
		...
		errCode := store.OverwriteUserProperties(projectID, user.ID, user.PropertiesId, userPropertiesJsonb)
		if errCode != http.StatusAccepted {
			logCtx.WithField("err_code", errCode).
				Error("Failed to overwrite user properties record.")
			hasFailure = true
...
func (pg *MemSQL) updateLatestUserPropertiesForSessionIfNotUpdatedV2(
	projectID uint64,
	sessionUpdateUserIDs map[string]bool,
	latestSessionUserPropertiesByUserID *map[string]LatestUserPropertiesFromSession,
) int {
