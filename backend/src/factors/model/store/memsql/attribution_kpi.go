...
func (store *MemSQL) ExecuteKPIForAttributionV1(projectID int64, query *model.AttributionKPIQueries, from int64, to int64, debugQueryKey string,
	...
	var kpiKeys []string
	var err error
	err, kpiKeys = store.RunKPIGroupQueryV1(projectID, query, from, to, &kpiData, enableOptimisedFilterOnProfileQuery,
		enableOptimisedFilterOnEventUserQuery, debugQueryKey, logCtx)
	if err != nil {
		return kpiData, err
	}
...
func (store *MemSQL) GetDataFromKPIResultV1(projectID int64, kpiQueryResult model.QueryResult, kpiData *map[string]model.KPIInfo, from int64, to int64, logCtx log.Entry) []string {
...
func (store *MemSQL) RunKPIGroupQueryV1(projectID int64, query *model.AttributionKPIQueries, from int64, to int64, kpiData *map[string]model.KPIInfo,
	...
	if query.AnalyzeType == model.AnalyzeTypeHSDeals || query.AnalyzeType == model.AnalyzeTypeSFOpportunities {
		...
		return nil, store.GetDataFromKPIResultV1(projectID, kpiQueryResult, kpiData, from, to, logCtx)
	}
	return errors.New("not a valid type of query for KPI Attribution"), nil
}
