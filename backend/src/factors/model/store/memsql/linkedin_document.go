...
const linkedinFilterQueryStr = "SELECT DISTINCT(JSON_EXTRACT_STRING(value, ?)) as filter_value FROM linkedin_documents WHERE project_id = ? AND" +
...
func (store *MemSQL) getLinkedinFilterValuesByType(projectID uint64, docType int, property string, reqID string) ([]interface{}, int) {
	...
	_, resultRows, _ := store.ExecuteSQL(linkedinFilterQueryStr, params, logCtx)
...
func (store *MemSQL) ExecuteLinkedinChannelQueryV1(projectID uint64, query *model.ChannelQueryV1, reqID string) ([]string, [][]interface{}, error) {
	fetchSource := false
	logCtx := log.WithField("xreq_id", reqID)
	sql, params, selectKeys, selectMetrics, err := store.GetSQLQueryAndParametersForLinkedinQueryV1(projectID, query, reqID, fetchSource)
	if err != nil {
...
func (store *MemSQL) GetSQLQueryAndParametersForLinkedinQueryV1(projectID uint64, query *model.ChannelQueryV1, reqID string, fetchSource bool) (string, []interface{}, []string, []string, error) {
