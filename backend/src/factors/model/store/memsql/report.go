...
func (store *MemSQL) CreateReport(report *model.Report) (*model.Report, int) {
...
func (store *MemSQL) DeleteReportByDashboardID(projectID, dashboardID uint64) int {
	db := C.GetServices().Db
...
func (store *MemSQL) GetReportByID(projectID, id uint64) (*model.Report, int) {
	if id == 0 {
...
func (store *MemSQL) GetReportsByProjectID(projectID uint64) ([]*model.Report, int) {
	if projectID == 0 {
		return nil, http.StatusBadRequest
	}
...
func (store *MemSQL) GetValidReportsListAgentHasAccessTo(projectID uint64,
	...
	dashboards, errCode := store.GetDashboards(projectID, agentUUID)
...
func (store *MemSQL) GenerateReport(projectID, dashboardID uint64, dashboardName string, reportType string,
	...
	dashboardUnits, errCode := store.GetDashboardUnitsByProjectIDAndDashboardIDAndTypes(
	...
	for _, dashboardUnit := range dashboardUnits {
		dashboardUnitReport, errCode := store.getDashboardUnitReport(projectID,
	...
	if err := store.addExplanationsAndOrderReportUnits(report); err != nil {
		return nil, http.StatusInternalServerError
	}
...
func (store *MemSQL) getInsightsReportUnit(projectID uint64, query model.Query, interval model.Interval) (*model.ReportUnit, int) {
	...
	queryResult, errCode, errMsg := store.Analyze(projectID, query)
	if errCode != http.StatusOK {
...
func (store *MemSQL) getChannelReportUnit(projectID uint64, channelQueryUnit model.ChannelQueryUnit,
	presentation string, interval model.Interval) (*model.ReportUnit, int) {

	logCtx := log.WithField("project_id", projectID)
	channelQueryResult, errCode := store.ExecuteChannelQuery(projectID, channelQueryUnit.Query)
	if errCode != http.StatusOK {
...
func (store *MemSQL) getTotalAggrForUniqueUsersQuery(projectId uint64, uuQuery model.Query) float64 {
	uuQuery.GroupByTimestamp = ""

	queryResult, _, _ := store.Analyze(projectId, uuQuery)
...
func (store *MemSQL) addExplanationsForPresentationLine(duReport *model.DashboardUnitReport, reportType string) error {
	prevResult := duReport.Results[0].QueryResult
	curResult := duReport.Results[1].QueryResult
	resultEntity := getEntityFromQueryType(duReport.Results[1].QueryResult.Meta.Query.Type)
	...
	if curResult.Meta.Query.Type == model.QueryTypeUniqueUsers {
		prevTotal = store.getTotalAggrForUniqueUsersQuery(duReport.ProjectID, prevResult.Meta.Query)
		curTotal = store.getTotalAggrForUniqueUsersQuery(duReport.ProjectID, curResult.Meta.Query)
	}
...
func (store *MemSQL) addExplanationsByPresentation(duReport model.DashboardUnitReport, reportType string) (model.DashboardUnitReport, error) {
	...
	case model.PresentationLine:
		err = store.addExplanationsForPresentationLine(&duReport, reportType)
...
func (store *MemSQL) addExplanationsAndOrderReportUnits(report *model.Report) (err error) {
	...
	for _, dashboardUnitReport := range report.Units {
		...
		report, err := store.addExplanationsByPresentation(dashboardUnitReport, report.Type)
		if err != nil {
...
func (store *MemSQL) getDashboardUnitReport(projectID uint64, dashboardUnit model.DashboardUnit, intervalBeforeThat,
	...
	if query.Class == model.QueryClassChannel {
		...
		intervalBeforeReportUnit, errCode := store.getChannelReportUnit(projectID,
			channelQueryUnit, dashboardUnit.Presentation, intervalBeforeThat)
		if errCode != http.StatusOK {
			return nil, errCode
		}

		intervalReportUnit, errCode := store.getChannelReportUnit(projectID,
		...
		intervalBeforeReportUnit, errCode := store.getInsightsReportUnit(projectID, query, intervalBeforeThat)
		if errCode != http.StatusOK {
			return nil, errCode
		}

		intervalReportUnit, errCode := store.getInsightsReportUnit(projectID, query, interval)
		if errCode != http.StatusOK {
			return nil, errCode
		}
