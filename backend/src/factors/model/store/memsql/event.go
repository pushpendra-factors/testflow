...
func (store *MemSQL) CreateEvent(event *model.Event) (*model.Event, int) {
	...
	rows, err := db.Raw("INSERT INTO events (id, customer_event_id,project_id,user_id,user_properties_id,session_id,event_name_id,count,properties,properties_updated_timestamp,timestamp,created_at,updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
	...
	for i := 0; i < len(events); {
		...
		if hasMarketingProperty || isNewSessionRequired || isLastSetOfEvents {
			...
			if events[sessionStartIndex].SessionId != nil {
				var errCode int
				existingSessionEvent, errCode = store.GetEventById(projectId, *events[sessionStartIndex].SessionId)
			...
			if existingSessionEvent != nil {
				...
				if firstEvent.UserPropertiesId != "" {
					userProperties, errCode := store.GetUserProperties(projectId, userId, firstEvent.UserPropertiesId)
					if errCode != http.StatusFound {
