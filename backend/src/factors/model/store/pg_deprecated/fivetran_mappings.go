...
func (pg *Postgres) DisableFiveTranMapping(ProjectID uint64, Integration string, ConnectorId string) error {
...
func (pg *Postgres) UpdateFiveTranMappingAccount(ProjectID uint64, Integration string, ConnectorId string, Accounts string) error {
...
func (pg *Postgres) GetFiveTranMapping(ProjectID uint64, Integration string) (string, error) {
	db := C.GetServices().Db
...
func (pg *Postgres) GetActiveFiveTranMapping(ProjectID uint64, Integration string) (model.FivetranMappings, error) {
	db := C.GetServices().Db
	var records model.FivetranMappings
...
func (pg *Postgres) GetLatestFiveTranMapping(ProjectID uint64, Integration string) (string, string, error) {
	db := C.GetServices().Db
	var records []model.FivetranMappings
	if err := db.Where("project_id = ?", ProjectID).Where("integration = ?", Integration).Find(&records).Error; err != nil {
		log.Error(err)
...
func (pg *Postgres) GetAllActiveFiveTranMapping(ProjectID uint64, Integration string) ([]string, error) {
	db := C.GetServices().Db
	var records []model.FivetranMappings
...
func (pg *Postgres) GetAllActiveFiveTranMappingByIntegration(Integration string) ([]model.FivetranMappings, error) {
	db := C.GetServices().Db
	var records []model.FivetranMappings
