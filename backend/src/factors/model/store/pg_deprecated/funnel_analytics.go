...
func buildUniqueUsersFunnelQuery(projectId uint64, q model.Query, groupIds []int) (string, []interface{}, error) {
	...
	for i := range q.EventsWithProperties {
		...
		if groupIds[i] != 0 {
			// For group level events the events.user_id contains group_user_id, hence the join.
			addJoinStatement = fmt.Sprintf("JOIN users ON users.group_%d_user_id = events.user_id AND users.project_id = ? ", groupIds[i])
		} else {
