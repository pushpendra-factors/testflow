...
func (pg *Postgres) GetNextArchivalBatches(projectID uint64, startTime int64, maxLookbackDays int, hardStartTime, hardEndTime time.Time) ([]model.EventsArchivalBatch, error) {
	...
	if !hardStartTime.IsZero() {
		...
		var status int
		completedBatches, status = pg.GetCompletedArchivalBatches(projectID, hardStartTime, hardEndTime)
