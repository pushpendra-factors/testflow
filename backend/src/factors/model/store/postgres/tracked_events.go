...
func (pg *Postgres) CreateFactorsTrackedEvent(ProjectID uint64, EventName string, agentUUID string) (int64, int) {
	if pg.isActiveFactorsTrackedEventsLimitExceeded(ProjectID) {
	...
	eventData, err := pg.GetEventName(EventName, ProjectID)
...
func (pg *Postgres) DeactivateFactorsTrackedEvent(ID int64, ProjectID uint64) (int64, int) {
	logCtx := log.WithFields(log.Fields{"project_id": ProjectID})
	transTime := gorm.NowFunc()
	existingFactorsTrackedEvent, dbErr := pg.GetFactorsTrackedEventByID(ID, ProjectID)
	if dbErr == nil {
		if existingFactorsTrackedEvent.IsActive == true {
			...
			eventDetails, err := pg.GetEventNameFromEventNameId(existingFactorsTrackedEvent.EventNameID, ProjectID)
			if err != nil {
				return 0, http.StatusBadRequest
			}
...
func (pg *Postgres) GetAllFactorsTrackedEventsByProject(ProjectID uint64) ([]model.FactorsTrackedEventInfo, int) {
	db := C.GetServices().Db
...
func (pg *Postgres) GetAllActiveFactorsTrackedEventsByProject(ProjectID uint64) ([]model.FactorsTrackedEventInfo, int) {
	db := C.GetServices().Db
...
func (pg *Postgres) GetFactorsTrackedEventByID(ID int64, ProjectID uint64) (*model.FactorsTrackedEvent, error) {
	logCtx := log.WithFields(log.Fields{"project_id": ProjectID})
	db := C.GetServices().Db
...
func (pg *Postgres) isActiveFactorsTrackedEventsLimitExceeded(ProjectID uint64) bool {
	trackedEvents, errCode := pg.GetAllActiveFactorsTrackedEventsByProject(ProjectID)
