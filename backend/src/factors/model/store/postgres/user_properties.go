...
func (pg *Postgres) BackFillUserDataInCacheFromDb(projectid uint64, currentDate time.Time,
	...
	begin := U.TimeNow()
	properties, err := pg.GetRecentUserPropertyKeysWithLimits(projectid, usersProcessedLimit, propertiesLimit, currentDate)
	end := U.TimeNow()
	...
	for _, propertyValue := range properties {
		...
		begin := U.TimeNow()
		values, category, err := pg.GetRecentUserPropertyValuesWithLimits(projectid, propertyValue.Key, usersProcessedLimit, valuesLimit, currentDate)
		end := U.TimeNow()
...
func (pg *Postgres) MergeUserPropertiesForProjectID(projectID uint64, dryRun bool) int {
	logCtx := log.WithFields(log.Fields{
		"Method":    "MergeUserPropertiesForProjectID",
		"ProjectID": projectID,
	})

	customerUserIDs, errCode := pg.GetDistinctCustomerUserIDSForProject(projectID)
	...
	for _, customerUserID := range customerUserIDs {
		...
		user, errCode := pg.GetUserLatestByCustomerUserId(projectID, customerUserID)
		if errCode != http.StatusFound {
			logCtx.Infof("Error or no users found for customer user id")
			continue
		}
		_, errCode = pg.MergeUserPropertiesForUserID(projectID, user.ID, postgres.Jsonb{}, "", U.TimeNowUnix(), dryRun, true)
		if errCode != http.StatusCreated && errCode != http.StatusNotModified {
...
func (pg *Postgres) MergeUserPropertiesForUserID(projectID uint64, userID string, updatedProperties postgres.Jsonb,
	...
	users, errCode := pg.getUsersForMergingPropertiesByCustomerUserID(projectID, customerUserID)
	...
	for _, user := range users {
		userPropertiesRecord, errCode := pg.GetUserPropertiesRecord(projectID, user.ID, user.PropertiesId)
		if errCode != http.StatusFound {
...
func (pg *Postgres) SanitizeAddTypeProperties(projectID uint64, users []model.User, propertiesMap *map[string]interface{}) {
	var userIDs []string
	for _, user := range users {
	...
	sessionEvent, errCode := pg.GetSessionEventName(projectID)
	if errCode != http.StatusFound {
		return
	}
	sessionCount, errCode := pg.GetEventCountOfUsersByEventName(projectID, userIDs, sessionEvent.ID)
	if errCode != http.StatusFound {
		return
	}
...
func (pg *Postgres) GetUserProperties(projectId uint64, userId string, id string) (*postgres.Jsonb, int) {
	userPropertiesRecord, errCode := pg.GetUserPropertiesRecord(projectId, userId, id)
...
func (pg *Postgres) GetUserPropertiesRecord(projectId uint64, userId string, id string) (*model.UserProperties, int) {
...
func (pg *Postgres) OverwriteUserProperties(projectId uint64, userId string,
...
func (pg *Postgres) GetUserPropertiesRecordsByProperty(projectId uint64,
...
func (pg *Postgres) UpdateUserPropertiesForSession(projectID uint64,
	...
	if C.ShouldUseUserPropertiesTableForRead(projectID) {
		return pg.updateUserPropertiesForSessionV1(projectID, sessionUserPropertiesRecordMap)
	}

	return pg.updateUserPropertiesForSessionV2(projectID, sessionUserPropertiesRecordMap)
}

func (pg *Postgres) updateUserPropertiesForSessionV1(projectID uint64,
	sessionUserPropertiesRecordMap *map[string]model.SessionUserProperties) int {
	...
	for userPropertiesID, sessionUserProperties := range *sessionUserPropertiesRecordMap {
		...
		userProperties, errCode := pg.GetUserPropertiesRecord(projectID,
		...
		errCode = pg.OverwriteUserProperties(projectID, sessionUserProperties.UserID,
		...
		if _, exists := updatedUsersByUserID[sessionUserProperties.UserID]; !exists {
			user, errCode := pg.GetUser(projectID, sessionUserProperties.UserID)
			if errCode != http.StatusFound {
	...
	errCode := pg.updateLatestUserPropertiesForSessionIfNotUpdatedV1(projectID,
...
func (pg *Postgres) updateLatestUserPropertiesForSessionIfNotUpdatedV1(projectID uint64,
	...
	for _, user := range sessionUsersByUserID {
		...
		errCode := pg.OverwriteUserProperties(projectID, user.ID, user.PropertiesId, userPropertiesJsonb)
		if errCode != http.StatusAccepted {
			logCtx.WithField("err_code", errCode).
				Error("Failed to overwrite user properties record.")
			hasFailure = true
