...
func (pg *Postgres) CreateDashboard(projectId uint64, agentUUID string, dashboard *model.Dashboard) (*model.Dashboard, int) {
	db := C.GetServices().Db
...
func (pg *Postgres) CreateAgentPersonalDashboardForProject(projectId uint64, agentUUID string) (*model.Dashboard, int) {
	return pg.CreateDashboard(projectId, agentUUID,
...
func (pg *Postgres) GetDashboards(projectId uint64, agentUUID string) ([]model.Dashboard, int) {
	db := C.GetServices().Db
...
func (pg *Postgres) GetDashboard(projectId uint64, agentUUID string, id uint64) (*model.Dashboard, int) {
	db := C.GetServices().Db
...
func (pg *Postgres) HasAccessToDashboard(projectId uint64, agentUUID string, id uint64) (bool, *model.Dashboard) {
	dashboard, errCode := pg.GetDashboard(projectId, agentUUID, id)
...
func (pg *Postgres) addUnitPositionOnDashboard(projectId uint64, agentUUID string,
	...
	return pg.UpdateDashboard(projectId, agentUUID, id,
...
func (pg *Postgres) removeUnitPositionOnDashboard(projectId uint64, agentUUID string,
...
func (pg *Postgres) UpdateDashboard(projectId uint64, agentUUID string, id uint64, dashboard *model.UpdatableDashboard) int {
	if projectId == 0 || agentUUID == "" || id == 0 {
	...
	if hasAccess, _ := pg.HasAccessToDashboard(projectId, agentUUID, id); !hasAccess {
...
func (pg *Postgres) DeleteDashboard(projectID uint64, agentUUID string, dashboardID uint64) int {
	db := C.GetServices().Db
	...
	hasAccess, _ := pg.HasAccessToDashboard(projectID, agentUUID, dashboardID)
	if !hasAccess {
		return http.StatusForbidden
	}

	dashboardUnits, errCode := pg.GetDashboardUnits(projectID, agentUUID, dashboardID)
	if errCode != http.StatusFound {
	...
	for _, dashboardUnit := range dashboardUnits {
		errCode := pg.deleteDashboardUnit(projectID, dashboardID, dashboardUnit.ID)
	...
	errCode = pg.DeleteReportByDashboardID(projectID, dashboardID)
	if errCode != http.StatusAccepted {
		log.WithFields(log.Fields{"project_id": projectID, "dashboard_id": dashboardID}).
...
func (pg *Postgres) createDefaultDashboardsForProject(projectId uint64, agentUUID string) int {
	return pg.CreateWebAnalyticsDefaultDashboardWithUnits(projectId, agentUUID)
}
