...
func (pg *Postgres) CreateSmartProperties(smartPropertiesDoc *model.SmartProperties) int {
...
func (pg *Postgres) UpdateSmartProperties(smartPropertiesDoc *model.SmartProperties) int {
	logCtx := log.WithField("project_id", smartPropertiesDoc.ProjectID)
...
func (pg *Postgres) GetSmartPropertiesByProjectIDAndSourceAndObjectType(projectID uint64, source string, objectType int) ([]model.SmartProperties, int) {
	db := C.GetServices().Db
...
func (pg *Postgres) DeleteSmartPropertyByProjectIDAndSourceAndObjectID(projectID uint64, source string, objectID string) int {
	db := C.GetServices().Db
...
func (pg *Postgres) GetSmartPropertyByProjectIDAndObjectIDAndObjectType(projectID uint64, objectID string, objectType int) (model.SmartProperties, int) {
	db := C.GetServices().Db
	smartProperties := model.SmartProperties{}
...
func (pg *Postgres) CreateSmartPropertiesFromChannelDocumentAndRule(smartPropertiesRule *model.SmartPropertiesRules, rule model.Rule,
	...
	smartProperty, errCode := pg.GetSmartPropertyByProjectIDAndObjectIDAndObjectType(smartPropertiesRule.ProjectID, objectID, smartPropertiesRule.Type)
	switch errCode {
	case http.StatusNotFound:
		...
		return pg.CreateSmartProperties(&smartProperties)
	...
	case http.StatusFound:
		...
		errCodeUpdate := pg.UpdateSmartProperties(&updatedSmartProperty)
...
func (pg *Postgres) DeleteSmartPropertiesByRuleID(projectID uint64, ruleID string) (int, int, int) {
	db := C.GetServices().Db
