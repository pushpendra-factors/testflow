...
func (pg *Postgres) CreateSmartProperty(smartPropertyDoc *model.SmartProperties) int {
...
func (pg *Postgres) UpdateSmartProperty(smartPropertyDoc *model.SmartProperties) int {
	logCtx := log.WithField("project_id", smartPropertyDoc.ProjectID)
...
func (pg *Postgres) GetSmartPropertyByProjectIDAndSourceAndObjectType(projectID uint64, source string, objectType int) ([]model.SmartProperties, int) {
	db := C.GetServices().Db
...
func (pg *Postgres) BuildAndCreateSmartPropertyFromChannelDocumentAndRule(smartPropertyRule *model.SmartPropertyRules, rule model.Rule,
	...
	smartProperty, errCode := pg.GetSmartPropertyByProjectIDAndObjectIDAndObjectType(smartPropertyRule.ProjectID, objectID, smartPropertyRule.Type)
	switch errCode {
	case http.StatusNotFound:
		...
		return pg.CreateSmartProperty(&smartProperty)
	...
	case http.StatusFound:
		...
		errCodeUpdate := pg.UpdateSmartProperty(&updatedSmartProperty)
...
func (pg *Postgres) DeleteSmartPropertyByRuleID(projectID uint64, ruleID string) (int, int, int) {
	db := C.GetServices().Db
