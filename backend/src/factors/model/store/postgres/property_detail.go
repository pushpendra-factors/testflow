...
func (pg *Postgres) CreatePropertyDetails(projectID uint64, eventName, propertyKey, propertyType string, isUserProperty bool, allowOverWrite bool) int {
	logCtx := log.WithFields(log.Fields{"project_id": projectID, "event_name": eventName, "property_key": propertyKey})
	...
	status, propertyDetail := pg.GetPropertyTypeFromDB(projectID, eventName, propertyKey, isUserProperty)
...
*/
func (pg *Postgres) CreateOrDeletePropertyDetails(projectID uint64, eventName, enKey, pType string, isUserProperty, allowOverWrite bool) error {
	...
	if pType == U.PropertyTypeUnknown { // unknown type would be deleted
		...
		status := pg.deletePropertyDetailsIfExist(projectID, eventName, enKey, isUserProperty)
	...
	status := pg.CreatePropertyDetails(projectID, eventName, enKey, pType, isUserProperty, allowOverWrite)
...
func (pg *Postgres) deletePropertyDetailsIfExist(projectID uint64, eventName, key string, isUserProperty bool) int {
	...
	status, propertyDetail := pg.GetPropertyTypeFromDB(projectID, eventName, key, isUserProperty)
	if status != http.StatusFound {
...
func (pg *Postgres) getPropertyDetailsForSmartEventName(projectID uint64, eventNameDetails *model.EventName) (*map[string]string, int) {
...
func (pg *Postgres) GetAllPropertyDetailsByProjectID(projectID uint64, eventName string, isUserProperty bool) (*map[string]string, int) {
	if projectID == 0 {
		return nil, http.StatusBadRequest
	}
	...
	if !isUserProperty {
		...
		if model.IsEventNameTypeSmartEvent(eventNameDetails.Type) {
			return pg.getPropertyDetailsForSmartEventName(projectID, eventNameDetails)
		}
