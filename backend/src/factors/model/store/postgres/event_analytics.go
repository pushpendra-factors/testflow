...
func (pg *Postgres) BuildInsightsQuery(projectId uint64, query model.Query) (string, []interface{}, error) {
...
func addEventFilterStepsForUniqueUsersQuery(projectID uint64, q *model.Query,
	...
	if q.GetGroupByTimestamp() != "" {
		...
		commonSelect = fmt.Sprintf("DISTINCT ON(coal_user_id%%, %s)", selectTimestamp) +
			fmt.Sprintf(" COALESCE(users.customer_user_id,events.user_id) as coal_user_id%%, %s as %s,", selectTimestamp, model.AliasDateTime) +
	...
	for i, ewp := range q.EventsWithProperties {
		...
		if q.EventsCondition == model.EventCondAllGivenEvent || q.EventsCondition == model.EventCondEachGivenEvent {
			...
			if hasGroupEntity(q.GroupByProperties, model.PropertyEntityEvent) {
...
*/
func addUniqueUsersAggregationQuery(query *model.Query, qStmnt *string,
	...
	if query.EventsCondition == model.EventCondAllGivenEvent {
		...
		eventGroupProps := filterGroupPropsByType(otherGroupBys, model.PropertyEntityEvent)
...
func buildUniqueUsersWithAnyGivenEventsQuery(projectID uint64,
	...
	eventGroupProps := filterGroupPropsByType(query.GroupByProperties, model.PropertyEntityEvent)
	_, _, egKeys := buildGroupKeys(eventGroupProps)
...
*/
func addEventCountAggregationQuery(query *model.Query, qStmnt *string,
	qParams *[]interface{}, refStep string) {
