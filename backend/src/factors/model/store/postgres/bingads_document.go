...
const (
	...
	bingadsFilterQueryStr          = "SELECT DISTINCT(LCASE(value->>?)) as filter_value FROM integration_documents WHERE project_id = ? AND customer_account_id IN (?) AND document_type = ? AND value->>? IS NOT NULL LIMIT 5000"
)
...
func (pg *Postgres) GetBingadsFilterValues(projectID uint64, requestFilterObject string, requestFilterProperty string, reqID string) ([]interface{}, int) {
	...
	_, resultRows, err := pg.ExecuteSQL(bingadsFilterQueryStr, params, logCtx)
	if err != nil {
...
func (pg *Postgres) GetBingadsFilterValuesSQLAndParams(projectID uint64, requestFilterObject string, requestFilterProperty string, reqID string) (string, []interface{}, int) {
	logFields := log.Fields{
		"project_id":              projectID,
		"request_filter_object":   requestFilterObject,
		"request_filter_property": requestFilterProperty,
...
func (pg *Postgres) buildObjectAndPropertiesForBingAds(projectID uint64, objects []string) []model.ChannelObjectAndProperties {
...
// modify
func (pg *Postgres) ExecuteBingAdsChannelQueryV1(projectID uint64, query *model.ChannelQueryV1, reqID string) ([]string, [][]interface{}, int) {
	logFields := log.Fields{
		"project_id": projectID,
		"query":      query,
		"req_id":     reqID,
	...
	if query.GroupByTimestamp == "" {
		sql, params, selectKeys, selectMetrics, errCode := pg.GetSQLQueryAndParametersForBingAdsQueryV1(projectID,
		...
		sql, params, selectKeys, selectMetrics, errCode := pg.GetSQLQueryAndParametersForBingAdsQueryV1(
...
func (pg *Postgres) GetSQLQueryAndParametersForBingAdsQueryV1(projectID uint64, query *model.ChannelQueryV1, reqID string, fetchSource bool,
	...
	logCtx := log.WithFields(logFields)
	transformedQuery, customerAccountID, err := pg.transFormRequestFieldsAndFetchRequiredFieldsForBingads(
...
func (pg *Postgres) transFormRequestFieldsAndFetchRequiredFieldsForBingads(projectID uint64,
