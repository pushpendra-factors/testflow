...
func (pg *Postgres) CreateWeeklyInsightsMetadata(wim *model.WeeklyInsightsMetadata) (int, string) {
	db := C.GetServices().Db
...
func (pg *Postgres) GetWeeklyInsightsMetadata(projectId uint64) ([]model.WeeklyInsightsMetadata, int, string) {
	db := C.GetServices().Db
