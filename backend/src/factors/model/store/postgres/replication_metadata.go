...
func (pg *Postgres) GetReplicationMetadataByTable(tableName string) (*model.ReplicationMetadata, int) {
	if tableName == "" {
...
func (pg *Postgres) CreateOrUpdateReplicationMetadataByTable(tableName string, lastRunAt *time.Time, count uint64) int {
