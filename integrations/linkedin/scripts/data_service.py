...
class DataService:
    ...
    def get_linkedin_int_settings_for_projects(self, project_ids):
        ...
        if not response.ok:
            ...
            return None, 'Failed to get linkedin integration settings for projects from data services'
        return response.json(), ''
    ...
    def get_last_sync_info(self, linkedin_setting, start_timestamp=None):
        ...
        for info in all_info:
            ...
            if start_timestamp != None:
                date = date = datetime.strptime(str(start_timestamp), '%Y%m%d')
            sync_info_with_type[info['type_alias']]= date.strftime('%Y-%m-%d')
        return sync_info_with_type, ''


    
    def add_linkedin_documents(self, project_id, ad_account_id, doc_type, obj_id, value, timestamp):
    ...
    def add_all_linkedin_documents(self, project_id, customer_acc_id, doc_type, docs, timestamp):
        #filling empty object when no data
        if len(docs) == 0:
            obj_id, empty_object = self.get_empty_object_with_req_ids()
            return self.add_linkedin_documents(project_id, customer_acc_id, doc_type, obj_id, empty_object, int(timestamp))
        response = {}
        for i in range(0, len(docs), BATCH_SIZE):
            batch = docs[i:i+BATCH_SIZE]
            response = self.add_multiple_linkedin_documents(project_id, customer_acc_id,
                                        doc_type,batch, timestamp)
            if not response.ok:
                return response
    ...
    def add_multiple_linkedin_documents(self, project_id, ad_account_id, doc_type, docs, timestamp):
        uri = '/data_service/linkedin/documents/add_multiple'
        url = self.data_service_host + uri

        batch_of_payloads = [self.get_payload_for_linkedin(project_id, ad_account_id,
    ...
    def get_payload_for_linkedin(self, project_id, ad_account_id, doc_type, value, timestamp):
        return {
            PROJECT_ID: int(project_id),
            'customer_ad_account_id': ad_account_id,
            'type_alias': doc_type,
