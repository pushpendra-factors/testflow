...
class AuthorisationFlow(BaseOauthService):
    ...
    def initialise_flow(self):
        flow = Flow.from_client_config(self.CONFIG.secret_json, scopes=["https://www.googleapis.com/auth/webmasters"])
        flow.redirect_uri = self.CONFIG.callback_url
        return flow

    def __init__(self, config=GSCOauthConfig):
        self.CONFIG = config
    ...
    def check_and_add_refresh_token(self, authorisation_code, state_payload):
        ...
        FactorsDataService.add_gsc_refresh_token(
            session,
            {
