...
def get_url_and_query_payload_from_gpt_response(gpt_response, pid):
    ...
    query_payload = transform_query(gpt_response, query_class)
...
def transform_query(gpt_response, query_class):
    query = None
    if query_class == "kpi":
        query = get_transformed_kpi_query(gpt_response)
    return query
