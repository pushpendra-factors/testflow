...
type SegmentProbMap struct {
	...
	UserDecoratorProbMap          map[string]map[string]RangeMapMultiplierTuple
...
func ComputeRangeMap(probMap map[string]float64, tag string) RangeMapMultiplierTuple {
	...
	if !(int(math.Round(sum*multiplier)) >= int(multiplier-5) && int(math.Round(sum*multiplier)) <= int(multiplier+5)) {
		Log.Error.Fatal("Probablity Sum != 1 ", tag, sum, multiplier)
	}
...
func PreComputeRangeMap(segment config.UserSegmentV2) SegmentProbMap {
	...
	probMap.UserDecoratorProbMap = make(map[string]map[string]RangeMapMultiplierTuple)
	...
	for item1, element1 := range segment.User_decorators {
		probMap.UserDecoratorProbMap[item1] = make(map[string]RangeMapMultiplierTuple)
		for item2, element2 := range element1 {
			probMap.UserDecoratorProbMap[item1][item2] =
				ComputeRangeMap(element2, fmt.Sprintf("%s-%s-%s", "UserDecorator", item1, item2))
		}
	}
